...
test("original leftover Ollama Claude-format ClaudeStreamHandler Unmarshal NewError gin.H", async () => {
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "messages"), true);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "messages", true), true);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "chat", true), true);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "completions", true), true);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "messages", false), false);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "responses", true), false);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "images", true), false);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_SUB2API, "messages", true), false);
  assert.equal(usesOllamaClaudeUnmarshal(CHANNEL_TYPE_OLLAMA, "messages"), true);
  assert.equal(usesSub2apiClaudeStreamUnmarshal(CHANNEL_TYPE_SUB2API, "messages", true), true);
  assert.equal(usesClaudeStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "chat", true), true);
  assert.equal(
  ...
  const claudeHelper = writeRelayNewAPIError(
    new Request("http://local/v1/messages", { headers: { "x-oneapi-request-id": "hop433-helper" } }),
    500,
    "invalid character 'o' looking for beginning of value",
    ERROR_CODE_BAD_RESPONSE_BODY,
  );
  ...
  assert.deepEqual(await claudeHelper.json(), {
    ...
    error: {
      type: ERROR_TYPE_NEW_API_ERROR,
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop433-helper"),
    },
  });

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.18" });
  await mergeModelRatio(new Store(e.DB), { "hop433-llama": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.19" },
      body: JSON.stringify({
        name: "hop433-ollama",
        type: CHANNEL_TYPE_OLLAMA,
        key: "ollama-hop433",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop415")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const claudeHit = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop433-ollama-unmarshal",
        },
        body: JSON.stringify({
          model: "hop433-llama",
          max_tokens: 32,
    ...
    assert.equal(
      claudeErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop433-ollama-unmarshal"),
    );
    assert.equal(claudeErr.code, undefined);
    assert.equal(claudeErr.param, undefined);
    ...
    const claudeArray = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop433-ollama-array",
        },
        body: JSON.stringify({
          model: "hop433-llama",
          max_tokens: 32,
    ...
    const nonStream = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop433-hop415-stay",
        },
        body: JSON.stringify({
          model: "hop433-llama",
          max_tokens: 32,
          messages: [{ role: "user", content: "non-stream-hop415" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      nonStreamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop433-hop415-stay"),
    );
    assert.equal(nonStreamErr.type, ERROR_TYPE_NEW_API_ERROR);
    ...
    const openaiStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.24", "x-oneapi-request-id": "hop433-openai-stream" },
        body: JSON.stringify({
          model: "hop433-llama",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
...
test("original leftover Ollama Claude-format ClaudeStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.25" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.26", "x-oneapi-request-id": "hop433-vendor-create" },
      body: JSON.stringify({ name: "hop433-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop433-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop433 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop433.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover Ali anthropic-messages ClaudeStreamHandler Unmarshal NewError gin.H", async () => {
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "messages", "hop434-qwen"), true);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "messages", "hop434-qwen", true), true);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "chat", "hop434-qwen", true), true);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "messages", "hop434-qwen", false), false);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "responses", "hop434-qwen", true), false);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "messages", "hop396-ali", true), false);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "images", "hop434-qwen", true), false);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "rerank", "hop434-qwen", true), false);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "messages", "hop434-qwen", true), false);
  assert.equal(usesAliClaudeUnmarshal(CHANNEL_TYPE_ALI, "messages", "hop434-qwen"), true);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "messages", true), true);
  assert.equal(usesClaudeStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "chat", true), true);
  assert.equal(
    claudeStreamSseUnmarshalError("data: not-json\n\n"),
  ...
  const claudeHelper = writeRelayNewAPIError(
    new Request("http://local/v1/messages", { headers: { "x-oneapi-request-id": "hop434-helper" } }),
    500,
    "invalid character 'o' looking for beginning of value",
    ERROR_CODE_BAD_RESPONSE_BODY,
  );
  ...
  assert.deepEqual(await claudeHelper.json(), {
    ...
    error: {
      type: ERROR_TYPE_NEW_API_ERROR,
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop434-helper"),
    },
  });

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.27" });
  await mergeModelRatio(new Store(e.DB), { "hop434-qwen": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.28" },
      body: JSON.stringify({
        name: "hop434-ali",
        type: CHANNEL_TYPE_ALI,
        key: "ali-hop434",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop416")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const claudeHit = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop434-ali-unmarshal",
        },
        body: JSON.stringify({
          model: "hop434-qwen",
          max_tokens: 32,
    ...
    assert.equal(
      claudeErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop434-ali-unmarshal"),
    );
    assert.equal(claudeErr.code, undefined);
    assert.equal(claudeErr.param, undefined);
    ...
    const claudeArray = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop434-ali-array",
        },
        body: JSON.stringify({
          model: "hop434-qwen",
          max_tokens: 32,
    ...
    const nonStream = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop434-hop416-stay",
        },
        body: JSON.stringify({
          model: "hop434-qwen",
          max_tokens: 32,
          messages: [{ role: "user", content: "non-stream-hop416" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      nonStreamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop434-hop416-stay"),
    );
    assert.equal(nonStreamErr.type, ERROR_TYPE_NEW_API_ERROR);
    ...
    const openaiStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.33", "x-oneapi-request-id": "hop434-openai-stream" },
        body: JSON.stringify({
          model: "hop434-qwen",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
...
test("original leftover Ali anthropic-messages ClaudeStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.34" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.35", "x-oneapi-request-id": "hop434-vendor-create" },
      body: JSON.stringify({ name: "hop434-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop434-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop434 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop434.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover Volc special-base ClaudeStreamHandler Unmarshal NewError gin.H", async () => {
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "messages", "doubao-coding-plan"), true);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "messages", "doubao-coding-plan", true), true);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "chat", "doubao-coding-plan", true), true);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "messages", "doubao-coding-plan", false), false);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "responses", "doubao-coding-plan", true), false);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "messages", "https://ark.cn-beijing.volces.com", true), false);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "completions", "doubao-coding-plan", true), false);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "audio_speech", "doubao-coding-plan", true), false);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "messages", "doubao-coding-plan", true), false);
  assert.equal(usesVolcClaudeUnmarshal(CHANNEL_TYPE_VOLC, "messages", "doubao-coding-plan"), true);
  assert.equal(usesAliClaudeStreamUnmarshal(CHANNEL_TYPE_ALI, "messages", "hop434-qwen", true), true);
  assert.equal(usesClaudeStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "chat", true), true);
  assert.equal(
  ...
  const claudeHelper = writeRelayNewAPIError(
    new Request("http://local/v1/messages", { headers: { "x-oneapi-request-id": "hop435-helper" } }),
    500,
    "invalid character 'o' looking for beginning of value",
    ERROR_CODE_BAD_RESPONSE_BODY,
  );
  ...
  assert.deepEqual(await claudeHelper.json(), {
    ...
    error: {
      type: ERROR_TYPE_NEW_API_ERROR,
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop435-helper"),
    },
  });

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.36" });
  await mergeModelRatio(new Store(e.DB), { "hop435-volc": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.37" },
      body: JSON.stringify({
        name: "hop435-volc",
        type: CHANNEL_TYPE_VOLC,
        key: "volc-hop435",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop417")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const claudeHit = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop435-volc-unmarshal",
        },
        body: JSON.stringify({
          model: "hop435-volc",
          max_tokens: 32,
    ...
    assert.equal(
      claudeErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop435-volc-unmarshal"),
    );
    assert.equal(claudeErr.code, undefined);
    assert.equal(claudeErr.param, undefined);
    ...
    const claudeArray = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...skAuth,
          "cf-connecting-ip": "198.51.100.39",
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop435-volc-array",
        },
        body: JSON.stringify({
          model: "hop435-volc",
          max_tokens: 32,
    ...
    const nonStream = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop435-hop417-stay",
        },
        body: JSON.stringify({
          model: "hop435-volc",
          max_tokens: 32,
          messages: [{ role: "user", content: "non-stream-hop417" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      nonStreamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop435-hop417-stay"),
    );
    assert.equal(nonStreamErr.type, ERROR_TYPE_NEW_API_ERROR);
    ...
    const openaiStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.42", "x-oneapi-request-id": "hop435-openai-stream" },
        body: JSON.stringify({
          model: "hop435-volc",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
...
test("original leftover Volc special-base ClaudeStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.43" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.44", "x-oneapi-request-id": "hop435-vendor-create" },
      body: JSON.stringify({ name: "hop435-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop435-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop435 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop435.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom ClaudeStreamHandler Unmarshal NewError gin.H", async () => {
  const chatToClaude = "openai_chat_completions_to_anthropic_messages";
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "openai"), true);
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "openai", true), true);
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "messages", "none", "anthropic", true), true);
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "openai", false), false);
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", chatToClaude, "openai", true), false);
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", "none", "openai", true), false);
  assert.equal(
    usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", "openai_chat_completions_to_gemini_generate_content", "openai", true),
    false,
  );
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "images", chatToClaude, "openai", true), false);
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "chat", chatToClaude, "openai", true), false);
  assert.equal(usesAdvancedCustomClaudeUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "openai"), true);
  assert.equal(usesVolcClaudeStreamUnmarshal(CHANNEL_TYPE_VOLC, "messages", "doubao-coding-plan", true), true);
  assert.equal(usesClaudeStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "chat", true), true);
  assert.equal(
  ...
  const chatHelper = writeRelayNewAPIError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop436-helper" } }),
    500,
    "invalid character 'o' looking for beginning of value",
    ERROR_CODE_BAD_RESPONSE_BODY,
  );
  ...
  assert.deepEqual(await chatHelper.json(), {
    error: {
      message: "invalid character 'o' looking for beginning of value (request id: hop436-helper)",
      type: ERROR_TYPE_NEW_API_ERROR,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.45" });
  await mergeModelRatio(new Store(e.DB), { "hop436-adv": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.46" },
      body: JSON.stringify({
        name: "hop436-adv",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop436",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop419")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const chatHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.47", "x-oneapi-request-id": "hop436-adv-unmarshal" },
        body: JSON.stringify({
          model: "hop436-adv",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
    ...
    assert.equal(
      chatErr.message,
      "invalid character 'o' looking for beginning of value (request id: hop436-adv-unmarshal)",
    );
    assert.equal(chatErr.type, ERROR_TYPE_NEW_API_ERROR);
    assert.equal(chatErr.param, "");
    assert.equal(chatErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const chatArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.48", "x-oneapi-request-id": "hop436-adv-array" },
        body: JSON.stringify({
          model: "hop436-adv",
          stream: true,
          messages: [{ role: "user", content: "as-array" }],
    ...
    assert.equal(
      chatArrayErr.message,
      "json: cannot unmarshal array into Go value of type dto.ClaudeResponse (request id: hop436-adv-array)",
    );
    assert.equal(chatArrayErr.type, ERROR_TYPE_NEW_API_ERROR);
    ...
    const claudeHit = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...skAuth,
          "cf-connecting-ip": "198.51.100.49",
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop436-adv-messages",
        },
        body: JSON.stringify({
          model: "hop436-adv",
          max_tokens: 32,
    ...
    assert.equal(
      claudeErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop436-adv-messages"),
    );
    assert.equal(claudeErr.code, undefined);
    assert.equal(claudeErr.param, undefined);
    ...
    const nonStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.50", "x-oneapi-request-id": "hop436-hop419-stay" },
        body: JSON.stringify({
          model: "hop436-adv",
          messages: [{ role: "user", content: "non-stream-hop419" }],
        }),
      }),
      e,
    );
    assert.equal(nonStream.res.status, 500, nonStream.text);
    const nonStreamErr = nonStream.body.error as { message: string; type: string };
    assert.equal(
      nonStreamErr.message,
      "invalid character 'o' looking for beginning of value (request id: hop436-hop419-stay)",
    );
    assert.equal(nonStreamErr.type, ERROR_TYPE_NEW_API_ERROR);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover advanced-custom ClaudeStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.51" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.52", "x-oneapi-request-id": "hop436-vendor-create" },
      body: JSON.stringify({ name: "hop436-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop436-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop436 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop436.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom GeminiChatStreamHandler Unmarshal NewOpenAIError gin.H", async () => {
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  const chatToClaude = "openai_chat_completions_to_anthropic_messages";
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai"), true);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop437-gemini", true), true);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "gemini", "hop437-gemini", true), true);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop437-gemini", false), false);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", chatToGemini, "openai", "hop437-gemini", true), false);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", "none", "openai", "hop437-gemini", true), false);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "openai", "hop437-gemini", true), false);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "images", chatToGemini, "openai", "hop437-gemini", true), false);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-3.0", true), false);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_GEMINI, "chat", chatToGemini, "openai", "hop437-gemini", true), false);
  assert.equal(usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai"), true);
  assert.equal(usesAdvancedCustomClaudeStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "openai", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop423-gemini", "chat", true), true);
  assert.equal(
  ...
  const chatHelper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop437-helper" } }),
    "unmarshal Gemini stream response: invalid character 'o' looking for beginning of value",
  );
  assert.equal(chatHelper.status, 500);
  assert.deepEqual(await chatHelper.json(), {
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.53" });
  await mergeModelRatio(new Store(e.DB), { "hop437-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.54" },
      body: JSON.stringify({
        name: "hop437-gemini",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop437",
        models: "hop437-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop420")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const chatHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.55", "x-oneapi-request-id": "hop437-adv-unmarshal" },
        body: JSON.stringify({
          model: "hop437-gemini",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
    ...
    const chatArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.56", "x-oneapi-request-id": "hop437-adv-array" },
        body: JSON.stringify({
          model: "hop437-gemini",
          stream: true,
          messages: [{ role: "user", content: "as-array" }],
    ...
    const nonStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.57", "x-oneapi-request-id": "hop437-hop420-stay" },
        body: JSON.stringify({
          model: "hop437-gemini",
          messages: [{ role: "user", content: "non-stream-hop420" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      nonStreamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop437-hop420-stay"),
    );
    assert.equal(nonStreamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover advanced-custom GeminiChatStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.58" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.59", "x-oneapi-request-id": "hop437-vendor-create" },
      body: JSON.stringify({ name: "hop437-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop437-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop437 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop437.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover Ollama OpenAI-format ollamaStreamHandler Unmarshal NewOpenAIError gin.H", async () => {
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "chat"), true);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "chat", true), true);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "completions", true), true);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "chat", false), false);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "embeddings", true), false);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "responses", true), false);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "images", true), false);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OPENAI, "chat", true), false);
  assert.equal(usesOllamaUnmarshal(CHANNEL_TYPE_OLLAMA, "chat"), true);
  assert.equal(usesOllamaClaudeStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "messages", true), true);
  assert.equal(usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", "openai_chat_completions_to_gemini_generate_content", "openai", "hop437-gemini", true), true);
  assert.equal(ollamaStreamUnmarshalError("not-json"), "invalid character 'o' looking for beginning of value");
  assert.equal(
    ollamaStreamUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type ollama.ollamaChatStreamChunk",
  );
  assert.equal(ollamaStreamUnmarshalError("null"), null);
  assert.equal(ollamaStreamUnmarshalError("{}"), null);
  assert.equal(ollamaStreamUnmarshalError("\nnot-json\n"), "invalid character 'o' looking for beginning of value");

  const chatHelper = writeOpenaiHandlerUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop438-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(chatHelper.status, 500);
  const helperBody = await chatHelper.json() as { error: { message: string; type: string; param: string; code: string } };
  assert.deepEqual(helperBody, {
    error: {
      message: "invalid character 'o' looking for beginning of value",
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
  ...
  assert.equal(helperBody.error.message.includes("hop438-helper"), false);

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.61" });
  await mergeModelRatio(new Store(e.DB), { "hop438-llama": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.62" },
      body: JSON.stringify({
        name: "hop438-ollama",
        type: CHANNEL_TYPE_OLLAMA,
        key: "ollama-hop438",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (raw.includes("non-stream-hop381")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
  ...
  try {
    const chatHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.63", "x-oneapi-request-id": "hop438-ollama-unmarshal" },
        body: JSON.stringify({
          model: "hop438-llama",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
    ...
    const chatErr = chatHit.body.error as { message: string; type: string; param: string; code: string };
    assert.equal(chatErr.message, "invalid character 'o' looking for beginning of value");
    assert.equal(chatErr.message.includes("hop438-ollama-unmarshal"), false);
    assert.equal(chatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(chatErr.param, "");
    assert.equal(chatErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const chatArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.64", "x-oneapi-request-id": "hop438-ollama-array" },
        body: JSON.stringify({
          model: "hop438-llama",
          stream: true,
          messages: [{ role: "user", content: "as-array" }],
    ...
    assert.equal(chatArrayErr.message.includes("hop438-ollama-array"), false);
    assert.equal(chatArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const nonStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.65", "x-oneapi-request-id": "hop438-hop381-stay" },
        body: JSON.stringify({
          model: "hop438-llama",
          messages: [{ role: "user", content: "non-stream-hop381" }],
        }),
      }),
      e,
    );
    assert.equal(nonStream.res.status, 500, nonStream.text);
    const nonStreamErr = nonStream.body.error as { message: string; type: string };
    assert.equal(nonStreamErr.message, "invalid character 'o' looking for beginning of value");
    assert.equal(nonStreamErr.message.includes("hop438-hop381-stay"), false);
    assert.equal(nonStreamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const claudeStream = await send(
      new Request("http://local/v1/messages", {
        method: "POST",
        headers: {
          ...skAuth,
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop438-hop433-stay",
        },
        body: JSON.stringify({
          model: "hop438-llama",
          max_tokens: 32,
    ...
    assert.equal(claudeStream.res.status, 500, claudeStream.text);
    assert.equal(claudeStream.body.type, "error", claudeStream.text);
    const claudeErr = claudeStream.body.error as { type: string; message: string };
    assert.equal(
      claudeErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop438-hop433-stay"),
    );
    assert.equal(claudeErr.type, ERROR_TYPE_NEW_API_ERROR);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover Ollama OpenAI-format ollamaStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.67" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.68", "x-oneapi-request-id": "hop438-vendor-create" },
      body: JSON.stringify({ name: "hop438-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop438-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop438 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop438.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover ClaudeResponsesStreamHandler Unmarshal FailResponsesStream / NewOpenAIError gin.H", async () => {
  assert.equal(usesClaudeResponsesStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "responses"), true);
  assert.equal(usesClaudeResponsesStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "responses", true), true);
  assert.equal(usesClaudeResponsesStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "responses", false), false);
  assert.equal(usesClaudeResponsesStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "chat", true), false);
  assert.equal(usesClaudeResponsesStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "messages", true), false);
  assert.equal(usesClaudeResponsesStreamUnmarshal(CHANNEL_TYPE_OPENAI, "responses", true), false);
  assert.equal(usesClaudeStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "chat", true), true);
  assert.equal(usesClaudeHandlerUnmarshal(CHANNEL_TYPE_ANTHROPIC, "responses"), true);
  assert.equal(usesOllamaStreamUnmarshal(CHANNEL_TYPE_OLLAMA, "chat", true), true);
  assert.equal(
    claudeResponsesStreamSseUnmarshalError("data: not-json\n\n"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    claudeResponsesStreamSseUnmarshalError("data: []\n\n"),
    "json: cannot unmarshal array into Go value of type dto.ClaudeResponse",
  );
  assert.equal(claudeResponsesStreamSseUnmarshalError("data: null\n\n"), null);
  assert.equal(claudeResponsesStreamSseUnmarshalError("data: {}\n\n"), null);

  const converted = claudeSseToResponsesSse("data: not-json\n\n", { id: "resp_hop439", model: "hop439-claude" });
  assert.equal(converted.sse.includes("failed to unmarshal Claude stream event"), false);
  assert.ok(converted.sse.includes("invalid character 'o' looking for beginning of value"), converted.sse);
  assert.ok(converted.sse.includes("event: error"), converted.sse);
  assert.ok(converted.sse.includes("response.failed"), converted.sse);
  assert.ok(converted.sse.includes('"code":"server_error"'), converted.sse);

  const convertedArray = claudeSseToResponsesSse("data: []\n\n", { id: "resp_hop439_array", model: "hop439-claude" });
  assert.ok(
    convertedArray.sse.includes("json: cannot unmarshal array into Go value of type dto.ClaudeResponse"),
    convertedArray.sse,
  );

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.69" });
  await mergeModelRatio(new Store(e.DB), { "hop439-claude": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.71" },
      body: JSON.stringify({
        name: "hop439-claude",
        type: CHANNEL_TYPE_ANTHROPIC,
        key: "sk-ant-hop439",
        models: "hop439-claude",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop406")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const responsesHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.72", "x-oneapi-request-id": "hop439-responses-unmarshal" },
        body: JSON.stringify({
          model: "hop439-claude",
          stream: true,
          input: "hello",
    ...
    assert.equal(responsesHit.res.status, 200, responsesHit.text);
    assert.equal(responsesHit.res.headers.get("content-type")?.includes("text/event-stream"), true, responsesHit.text);
    assert.equal(responsesHit.text.includes("failed to unmarshal Claude stream event"), false, responsesHit.text);
    ...
    const responsesArray = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.73", "x-oneapi-request-id": "hop439-responses-array" },
        body: JSON.stringify({
          model: "hop439-claude",
          stream: true,
          input: "as-array",
    ...
    assert.equal(responsesArray.res.status, 200, responsesArray.text);
    assert.ok(
      responsesArray.text.includes("json: cannot unmarshal array into Go value of type dto.ClaudeResponse"),
      responsesArray.text,
    );

    const messagesStream = await send(
      new Request("http://local/v1/messages", {
        method: "POST",
        headers: {
          ...skAuth,
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop439-hop422-stay",
        },
        body: JSON.stringify({
          model: "hop439-claude",
          max_tokens: 32,
    ...
    assert.equal(messagesStream.res.status, 500, messagesStream.text);
    assert.equal(messagesStream.body.type, "error", messagesStream.text);
    const messagesErr = messagesStream.body.error as { type: string; message: string };
    assert.equal(
      messagesErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop439-hop422-stay"),
    );
    assert.equal(messagesErr.type, ERROR_TYPE_NEW_API_ERROR);
    ...
    const nonStream = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...
          "anthropic-version": "2023-06-01",
          "x-oneapi-request-id": "hop439-hop406-stay",
        },
        body: JSON.stringify({
          model: "hop439-claude",
          max_tokens: 32,
          messages: [{ role: "user", content: "non-stream-hop406" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      nonStreamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop439-hop406-stay"),
    );
    assert.equal(nonStreamErr.type, ERROR_TYPE_NEW_API_ERROR);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover ClaudeResponsesStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.76" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.77", "x-oneapi-request-id": "hop439-vendor-create" },
      body: JSON.stringify({ name: "hop439-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop439-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop439 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop439.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover GeminiResponsesStreamHandler Unmarshal FailResponsesStream / NewOpenAIError gin.H", async () => {
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop440-gemini", "responses"), true);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop440-gemini", "responses", true), true);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop440-gemini", "responses", false), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop440-gemini", "chat", true), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop440-gemini", "gemini", true), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_OPENAI, "hop440-gemini", "responses", true), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "hop440-gemini", "responses", true), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-3.0", "responses", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop440-gemini", "chat", true), true);
  assert.equal(usesClaudeResponsesStreamUnmarshal(CHANNEL_TYPE_ANTHROPIC, "responses", true), true);
  assert.equal(
    geminiResponsesStreamSseUnmarshalError("data: not-json\n\n"),
    "unmarshal Gemini stream response: invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    geminiResponsesStreamSseUnmarshalError("data: []\n\n"),
    "unmarshal Gemini stream response: json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
  );
  assert.equal(geminiResponsesStreamSseUnmarshalError("data: null\n\n"), null);
  assert.equal(geminiResponsesStreamSseUnmarshalError("data: {}\n\n"), null);

  const converted = geminiSseToResponsesSse("data: not-json\n\n", { id: "resp_hop440", model: "hop440-gemini" });
  assert.ok(
    converted.sse.includes("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value"),
    converted.sse,
  );
  assert.ok(converted.sse.includes("event: error"), converted.sse);
  assert.ok(converted.sse.includes("response.failed"), converted.sse);
  ...
  const convertedArray = geminiSseToResponsesSse("data: []\n\n", { id: "resp_hop440_array", model: "hop440-gemini" });
  assert.ok(
    convertedArray.sse.includes(
      "unmarshal Gemini stream response: json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
    ),
    convertedArray.sse,
  );
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.78" });
  await mergeModelRatio(new Store(e.DB), { "hop440-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.79" },
      body: JSON.stringify({
        name: "hop440-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop440",
        models: "hop440-gemini",
        group: "default",
      }),
    }),
    e,
  ...
  try {
    const responsesHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.81", "x-oneapi-request-id": "hop440-responses-unmarshal" },
        body: JSON.stringify({
          model: "hop440-gemini",
          stream: true,
          input: "hello",
    ...
    assert.equal(responsesHit.res.headers.get("content-type")?.includes("text/event-stream"), true, responsesHit.text);
    assert.ok(
      responsesHit.text.includes("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value"),
      responsesHit.text,
    );
    assert.ok(responsesHit.text.includes("event: error"), responsesHit.text);
    assert.ok(responsesHit.text.includes("response.failed"), responsesHit.text);
    ...
    const responsesArray = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.82", "x-oneapi-request-id": "hop440-responses-array" },
        body: JSON.stringify({
          model: "hop440-gemini",
          stream: true,
          input: "as-array",
    ...
    const chatStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.83", "x-oneapi-request-id": "hop440-hop423-stay" },
        body: JSON.stringify({
          model: "hop440-gemini",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
    ...
    assert.equal(chatStream.res.status, 500, chatStream.text);
    const chatErr = chatStream.body.error as { message: string; type: string; code: string };
    assert.equal(
      chatErr.message,
      messageWithRequestId(
        "unmarshal Gemini stream response: invalid character 'o' looking for beginning of value",
    ...
    const nonStream = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.84", "x-oneapi-request-id": "hop440-hop360-stay" },
        body: JSON.stringify({ model: "hop440-gemini", messages: [{ role: "user", content: "non-stream-hop360" }] }),
      }),
      e,
    );
    assert.equal(nonStream.res.status, 500, nonStream.text);
    ...
    assert.equal(
      nonStreamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop440-hop360-stay"),
    );
    assert.equal(nonStreamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover GeminiResponsesStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.85" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.86", "x-oneapi-request-id": "hop440-vendor-create" },
      body: JSON.stringify({ name: "hop440-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop440-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop440 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop440.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom GeminiResponsesStreamHandler Unmarshal FailResponsesStream / NewOpenAIError gin.H", async () => {
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  assert.equal(
    usesAdvancedCustomGeminiResponsesStreamUnmarshal(
      CHANNEL_TYPE_ADVANCED_CUSTOM,
      "responses",
      responsesToGemini,
      "openai",
  ...
  assert.equal(
    usesAdvancedCustomGeminiStreamUnmarshal(
      CHANNEL_TYPE_ADVANCED_CUSTOM,
      "responses",
      responsesToGemini,
      "openai",
  ...
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop440-gemini", "responses", true), true);
  assert.equal(
    advancedCustomGeminiResponsesStreamSseUnmarshalError("data: not-json\n\n"),
    "unmarshal Gemini stream response: invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    advancedCustomGeminiResponsesStreamSseUnmarshalError("data: []\n\n"),
    "unmarshal Gemini stream response: json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
  );
  assert.equal(advancedCustomGeminiResponsesStreamSseUnmarshalError("data: null\n\n"), null);

  const converted = geminiSseToResponsesSse("data: not-json\n\n", { id: "resp_hop441", model: "hop441-gemini" });
  assert.ok(
    converted.sse.includes("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value"),
    converted.sse,
  );
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.87" });
  await mergeModelRatio(new Store(e.DB), { "hop441-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.88" },
      body: JSON.stringify({
        name: "hop441-gemini",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop441",
        models: "hop441-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop421")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const responsesHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.89", "x-oneapi-request-id": "hop441-adv-unmarshal" },
        body: JSON.stringify({
          model: "hop441-gemini",
          stream: true,
          input: "hello",
    ...
    const responsesArray = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.91", "x-oneapi-request-id": "hop441-adv-array" },
        body: JSON.stringify({
          model: "hop441-gemini",
          stream: true,
          input: "as-array",
    ...
    const nonStream = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.92", "x-oneapi-request-id": "hop441-hop421-stay" },
        body: JSON.stringify({ model: "hop441-gemini", input: "non-stream-hop421" }),
      }),
      e,
    );
    assert.equal(nonStream.res.status, 500, nonStream.text);
    ...
    assert.equal(
      nonStreamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop441-hop421-stay"),
    );
    assert.equal(nonStreamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover advanced-custom GeminiResponsesStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.93" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.94", "x-oneapi-request-id": "hop441-vendor-create" },
      body: JSON.stringify({ name: "hop441-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop441-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop441 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop441.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover OaiChatToResponsesStreamHandler Unmarshal FailResponsesStream / NewOpenAIError gin.H", async () => {
  const responsesToChat = "openai_responses_to_openai_chat_completions";
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  assert.equal(
    usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat),
    true,
  );
  assert.equal(
    usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, true),
    true,
  );
  assert.equal(
    usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, false),
    false,
  );
  assert.equal(
    usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", responsesToChat, true),
    false,
  );
  assert.equal(
    usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, true),
    false,
  );
  assert.equal(
    usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", "none", true),
    false,
  );
  assert.equal(
    usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_OPENAI, "responses", responsesToChat, true),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiResponsesStreamUnmarshal(
  ...
  assert.equal(
    oaiChatToResponsesStreamSseUnmarshalError("data: not-json\n\n"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    oaiChatToResponsesStreamSseUnmarshalError("data: []\n\n"),
    "json: cannot unmarshal array into Go value of type dto.ChatCompletionsStreamResponse",
  );
  assert.equal(oaiChatToResponsesStreamSseUnmarshalError("data: null\n\n"), null);
  assert.equal(oaiChatToResponsesStreamSseUnmarshalError("data: {}\n\n"), null);

  const converted = oaiChatSseToResponsesSse("data: not-json\n\n", { id: "resp_hop442", model: "hop442-chat" });
  assert.equal(converted.sse.includes("failed to unmarshal chat stream response"), false, converted.sse);
  assert.ok(converted.sse.includes("invalid character 'o' looking for beginning of value"), converted.sse);
  assert.ok(converted.sse.includes("event: error"), converted.sse);
  assert.ok(converted.sse.includes("response.failed"), converted.sse);
  assert.ok(converted.sse.includes('"code":"server_error"'), converted.sse);

  const convertedArray = oaiChatSseToResponsesSse("data: []\n\n", { id: "resp_hop442_array", model: "hop442-chat" });
  assert.ok(
    convertedArray.sse.includes("json: cannot unmarshal array into Go value of type dto.ChatCompletionsStreamResponse"),
    convertedArray.sse,
  );

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.95" });
  await mergeModelRatio(new Store(e.DB), { "hop442-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.96" },
      body: JSON.stringify({
        name: "hop442-chat",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop442",
  ...
  try {
    const responsesHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.97", "x-oneapi-request-id": "hop442-chat-unmarshal" },
        body: JSON.stringify({
          model: "hop442-chat",
          stream: true,
          input: "hello",
    ...
    assert.equal(responsesHit.res.status, 200, responsesHit.text);
    assert.equal(responsesHit.res.headers.get("content-type")?.includes("text/event-stream"), true, responsesHit.text);
    assert.equal(responsesHit.text.includes("failed to unmarshal chat stream response"), false, responsesHit.text);
    assert.ok(responsesHit.text.includes("invalid character 'o' looking for beginning of value"), responsesHit.text);
    assert.ok(responsesHit.text.includes("event: error"), responsesHit.text);
    assert.ok(responsesHit.text.includes("response.failed"), responsesHit.text);
    assert.equal(
      responsesHit.text.includes(
        messageWithRequestId("invalid character 'o' looking for beginning of value", "hop442-chat-unmarshal"),
      ),
      false,
      responsesHit.text,
    );
    ...
    const responsesArray = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.98", "x-oneapi-request-id": "hop442-chat-array" },
        body: JSON.stringify({
          model: "hop442-chat",
          stream: true,
          input: "as-array",
    ...
    assert.equal(responsesArray.res.status, 200, responsesArray.text);
    assert.ok(
      responsesArray.text.includes("json: cannot unmarshal array into Go value of type dto.ChatCompletionsStreamResponse"),
      responsesArray.text,
    );
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover OaiChatToResponsesStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.99" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.101", "x-oneapi-request-id": "hop442-vendor-create" },
      body: JSON.stringify({ name: "hop442-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop442-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop442 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop442.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover OaiChatToResponsesHandler Unmarshal NewOpenAIError gin.H", async () => {
  const responsesToChat = "openai_responses_to_openai_chat_completions";
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat), true);
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, false), true);
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, true), false);
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", responsesToChat, false), false);
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, false), false);
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_OPENAI, "responses", responsesToChat, false), false);
  assert.equal(usesOaiChatToResponsesStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, true), true);
  assert.equal(
    oaiChatToResponsesResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    oaiChatToResponsesResponseUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type dto.OpenAITextResponse",
  );
  assert.equal(oaiChatToResponsesResponseUnmarshalError("null"), null);

  const helper = writeOpenaiHandlerUnmarshalError(
    new Request("http://local/v1/responses", { headers: { "x-oneapi-request-id": "hop443-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.102" });
  await mergeModelRatio(new Store(e.DB), { "hop443-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.103" },
      body: JSON.stringify({
        name: "hop443-chat",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop443",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("stream-hop442-stay")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    if (raw.includes("as-array")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
  ...
  try {
    const hit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.104", "x-oneapi-request-id": "hop443-chat-unmarshal" },
        body: JSON.stringify({ model: "hop443-chat", input: "hello" }),
      }),
      e,
    );
    assert.equal(hit.res.status, 500, hit.text);
    assert.equal("type" in hit.body && hit.body.type === "error", false, hit.text);
    const err = hit.body.error as { message: string; type: string; param: string; code: string };
    assert.equal(err.message, "invalid character 'o' looking for beginning of value");
    assert.equal(err.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(err.param, "");
    assert.equal(err.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(hit.text.includes("hop443-chat-unmarshal"), false, hit.text);

    const arr = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.105", "x-oneapi-request-id": "hop443-chat-array" },
        body: JSON.stringify({ model: "hop443-chat", input: "as-array" }),
      }),
      e,
    );
    assert.equal(arr.res.status, 500, arr.text);
    ...
    const streamHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.106", "x-oneapi-request-id": "hop443-hop442-stay" },
        body: JSON.stringify({ model: "hop443-chat", stream: true, input: "stream-hop442-stay" }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 200, streamHit.text);
    assert.ok(streamHit.text.includes("invalid character 'o' looking for beginning of value"), streamHit.text);
    assert.ok(streamHit.text.includes("event: error"), streamHit.text);
    assert.ok(streamHit.text.includes("response.failed"), streamHit.text);
  } finally {
    globalThis.fetch = origFetch;
  }
});

test("original leftover OaiChatToResponsesHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.107" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.108", "x-oneapi-request-id": "hop443-vendor-create" },
      body: JSON.stringify({ name: "hop443-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop443-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop443 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop443.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover OaiResponsesToChatStreamHandler Unmarshal NewOpenAIError gin.H", async () => {
  const chatToResponses = "openai_chat_completions_to_openai_responses";
  const responsesToChat = "openai_responses_to_openai_chat_completions";
  assert.equal(
    usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses),
    true,
  );
  assert.equal(
    usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses, true),
    true,
  );
  assert.equal(
    usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "completions", chatToResponses, true),
    true,
  );
  assert.equal(
    usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses, false),
    false,
  );
  assert.equal(
    usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", chatToResponses, true),
    false,
  );
  assert.equal(
    usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", responsesToChat, true),
    false,
  );
  assert.equal(
    usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_OPENAI, "chat", chatToResponses, true),
    false,
  );
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, false), true);
  assert.equal(
    oaiResponsesToChatStreamSseUnmarshalError("data: not-json\n\n"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    oaiResponsesToChatStreamSseUnmarshalError("data: []\n\n"),
    "json: cannot unmarshal array into Go value of type dto.ResponsesStreamResponse",
  );
  assert.equal(oaiResponsesToChatStreamSseUnmarshalError("data: null\n\n"), null);
  assert.equal(oaiResponsesToChatStreamSseUnmarshalError("data: {}\n\n"), null);

  const helper = writeOpenaiHandlerUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop444-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.109" });
  await mergeModelRatio(new Store(e.DB), { "hop444-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.111" },
      body: JSON.stringify({
        name: "hop444-chat",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop444",
  ...
  try {
    const chatHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.112", "x-oneapi-request-id": "hop444-chat-unmarshal" },
        body: JSON.stringify({
          model: "hop444-chat",
          stream: true,
          messages: [{ role: "user", content: "hello" }],
    ...
    assert.equal(chatErr.param, "");
    assert.equal(chatErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(chatHit.text.includes("hop444-chat-unmarshal"), false, chatHit.text);

    const chatArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.113", "x-oneapi-request-id": "hop444-chat-array" },
        body: JSON.stringify({
          model: "hop444-chat",
          stream: true,
          messages: [{ role: "user", content: "as-array" }],
...
test("original leftover OaiResponsesToChatStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.114" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.115", "x-oneapi-request-id": "hop444-vendor-create" },
      body: JSON.stringify({ name: "hop444-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop444-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop444 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop444.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom OaiResponsesToChatHandler Unmarshal NewOpenAIError gin.H", async () => {
  const chatToResponses = "openai_chat_completions_to_openai_responses";
  const responsesToChat = "openai_responses_to_openai_chat_completions";
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses), true);
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses, false), true);
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "completions", chatToResponses, false), true);
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses, true), false);
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", chatToResponses, false), false);
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", responsesToChat, false), false);
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_OPENAI, "chat", chatToResponses, false), false);
  assert.equal(usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses, true), true);
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, false), true);
  assert.equal(
    oaiResponsesToChatResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    oaiResponsesToChatResponseUnmarshalError("[]"),
  ...
  assert.equal(oaiResponsesToChatResponseUnmarshalError("null"), null);

  const helper = writeOpenaiHandlerUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop445-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.116" });
  await mergeModelRatio(new Store(e.DB), { "hop445-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.117" },
      body: JSON.stringify({
        name: "hop445-chat",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop445",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("stream-hop444-stay")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    if (raw.includes("as-array")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
  ...
  try {
    const chatHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.118", "x-oneapi-request-id": "hop445-chat-unmarshal" },
        body: JSON.stringify({
          model: "hop445-chat",
          messages: [{ role: "user", content: "hello" }],
        }),
    ...
    assert.equal(chatErr.param, "");
    assert.equal(chatErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(chatHit.text.includes("hop445-chat-unmarshal"), false, chatHit.text);

    const chatArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.119", "x-oneapi-request-id": "hop445-chat-array" },
        body: JSON.stringify({
          model: "hop445-chat",
          messages: [{ role: "user", content: "as-array" }],
        }),
    ...
    const streamHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.121", "x-oneapi-request-id": "hop445-hop444-stay" },
        body: JSON.stringify({
          model: "hop445-chat",
          stream: true,
          messages: [{ role: "user", content: "stream-hop444-stay" }],
        }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 500, streamHit.text);
    const streamErr = streamHit.body.error as { message: string; type: string };
    assert.equal(streamErr.message, "invalid character 'o' looking for beginning of value");
    assert.equal(streamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(streamHit.text.includes("hop445-hop444-stay"), false, streamHit.text);
  } finally {
    globalThis.fetch = origFetch;
  }
});

test("original leftover advanced-custom OaiResponsesToChatHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.122" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.123", "x-oneapi-request-id": "hop445-vendor-create" },
      body: JSON.stringify({ name: "hop445-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop445-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop445 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop445.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover OaiResponsesToChatBufferedStreamHandler Unmarshal NewOpenAIError gin.H", async () => {
  const chatToResponses = "openai_chat_completions_to_openai_responses";
  const responsesToChat = "openai_responses_to_openai_chat_completions";
  assert.equal(usesOaiResponsesToChatBufferedStreamUnmarshal(true), true);
  assert.equal(usesOaiResponsesToChatBufferedStreamUnmarshal(true, false), true);
  assert.equal(usesOaiResponsesToChatBufferedStreamUnmarshal(true, true), false);
  assert.equal(usesOaiResponsesToChatBufferedStreamUnmarshal(false, false), false);
  assert.equal(usesOaiResponsesToChatUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses, false), true);
  assert.equal(usesOaiResponsesToChatStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToResponses, true), true);
  assert.equal(usesOaiChatToResponsesUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToChat, false), true);
  assert.equal(openaiDoResponseUnmarshalMode("chat", true), "responses");
  assert.equal(
    oaiResponsesToChatBufferedStreamSseUnmarshalError("data: not-json\n\n"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    oaiResponsesToChatBufferedStreamSseUnmarshalError("data: []\n\n"),
    "json: cannot unmarshal array into Go value of type dto.ResponsesStreamResponse",
  );
  assert.equal(oaiResponsesToChatBufferedStreamSseUnmarshalError("data: null\n\n"), null);
  assert.equal(oaiResponsesToChatBufferedStreamSseUnmarshalError("data: {}\n\n"), null);

  const helper = writeOpenaiHandlerUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop446-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
  ...
  const claudeHelper = writeOpenaiHandlerUnmarshalError(
    new Request("http://local/v1/messages", { headers: { "x-oneapi-request-id": "hop446-helper-claude" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(claudeHelper.status, 500);
  assert.deepEqual(await claudeHelper.json(), {
    ...
    error: {
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop446-helper-claude"),
    },
  });

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.124" });
  await mergeModelRatio(new Store(e.DB), { "hop446-via-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const policy = await send(
    new Request("http://local/api/option/", {
      method: "PUT",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.125" },
      body: JSON.stringify({
        key: "global.chat_completions_to_responses_policy",
        value: JSON.stringify({ enabled: true, all_channels: true, model_patterns: ["^hop446-via-chat$"] }),
      }),
    }),
    e,
  );
  ...
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.126" },
      body: JSON.stringify({
        name: "hop446-via",
        type: CHANNEL_TYPE_OPENAI,
        key: "sk-hop446",
        models: "hop446-via-chat",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("as-array-hop371-stay")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("data: []\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
  ...
  try {
    const chatHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.127", "x-oneapi-request-id": "hop446-chat-unmarshal" },
        body: JSON.stringify({
          model: "hop446-via-chat",
          messages: [{ role: "user", content: "hello" }],
        }),
      }),
      e,
    ...
    assert.equal(chatErr.param, "");
    assert.equal(chatErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(chatHit.text.includes("hop446-chat-unmarshal"), false, chatHit.text);
    assert.equal(chatHit.text.includes("failed to unmarshal buffered responses stream event"), false, chatHit.text);

    const chatArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.128", "x-oneapi-request-id": "hop446-chat-array" },
        body: JSON.stringify({
          model: "hop446-via-chat",
          messages: [{ role: "user", content: "as-array" }],
        }),
    ...
    const hop371Stay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.129", "x-oneapi-request-id": "hop446-hop371-stay" },
        body: JSON.stringify({
          model: "hop446-via-chat",
          messages: [{ role: "user", content: "as-array-hop371-stay" }],
        }),
      }),
      e,
    );
    assert.equal(hop371Stay.res.status, 500, hop371Stay.text);
    const hop371Err = hop371Stay.body.error as { message: string; type: string };
    assert.equal(
      hop371Err.message,
      "json: cannot unmarshal array into Go value of type dto.OpenAIResponsesResponse",
    );
    assert.equal(hop371Err.type, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const viaClaude = await send(
      new Request("http://local/v1/messages", {
        ...
        headers: {
          ...skAuth,
          "cf-connecting-ip": "198.51.100.131",
          "x-oneapi-request-id": "hop446-via-claude",
          "anthropic-version": "2023-06-01",
        },
        body: JSON.stringify({
          model: "hop446-via-chat",
    ...
    assert.equal(viaClaudeErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      viaClaudeErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop446-via-claude"),
    );
    assert.equal(viaClaudeErr.code, undefined);
    assert.equal(viaClaudeErr.param, undefined);
  } finally {
...
test("original leftover OaiResponsesToChatBufferedStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.132" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.133", "x-oneapi-request-id": "hop446-vendor-create" },
      body: JSON.stringify({ name: "hop446-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop446-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop446 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop446.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover openaiImageJSONAsStreamHandler Unmarshal NewOpenAIError gin.H", async () => {
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "images"), true);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "images", true), true);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "images", true, "application/json"), true);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "images", true, ""), true);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "images", false, "application/json"), false);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "images", true, "text/event-stream"), false);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "chat", true, "application/json"), false);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_ALI, "images", true, "application/json"), false);
  assert.equal(usesOaiResponsesToChatBufferedStreamUnmarshal(true, false), true);
  assert.equal(
    openaiImageJSONAsStreamResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    openaiImageJSONAsStreamResponseUnmarshalError("[]"),
  ...
  assert.equal(openaiImageJSONAsStreamResponseUnmarshalError("null"), null);

  const helper = writeOpenaiHandlerUnmarshalError(
    new Request("http://local/v1/images/generations", { headers: { "x-oneapi-request-id": "hop447-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.134" });
  await mergeModelRatio(new Store(e.DB), { "hop447-image": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.135" },
      body: JSON.stringify({
        name: "hop447-image",
        type: CHANNEL_TYPE_OPENAI,
        key: "sk-hop447",
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop365-stay")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
  ...
  try {
    const imageHit = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.136", "x-oneapi-request-id": "hop447-image-unmarshal" },
        body: JSON.stringify({
          model: "hop447-image",
          prompt: "hello",
          stream: true,
    ...
    assert.equal(imageErr.param, "");
    assert.equal(imageErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(imageHit.text.includes("hop447-image-unmarshal"), false, imageHit.text);

    const imageArray = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.137", "x-oneapi-request-id": "hop447-image-array" },
        body: JSON.stringify({
          model: "hop447-image",
          prompt: "as-array",
          stream: true,
    ...
    assert.equal(imageArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const hop365Stay = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.138", "x-oneapi-request-id": "hop447-hop365-stay" },
        body: JSON.stringify({
          model: "hop447-image",
          prompt: "non-stream-hop365-stay",
        }),
      }),
      e,
    );
...
test("original leftover openaiImageJSONAsStreamHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.139" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.141", "x-oneapi-request-id": "hop447-vendor-create" },
      body: JSON.stringify({ name: "hop447-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop447-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop447 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop447.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover GeminiImageHandler Unmarshal NewOpenAIError gin.H", async () => {
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop448"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop448", false), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop448", true), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_VERTEX, "imagen-3.0-generate-001", false), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_VERTEX, "imagen-3.0-generate-001", true), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "gemini-hop448-chat", false), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_OPENAI, "imagen-hop448", false), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop448", "images", true), false);
  assert.equal(usesOpenaiImageJSONAsStreamUnmarshal(CHANNEL_TYPE_OPENAI, "images", true, "application/json"), true);
  assert.equal(
    geminiImageResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    geminiImageResponseUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type dto.GeminiImageResponse",
  );
  assert.equal(geminiImageResponseUnmarshalError("null"), null);
  assert.equal(geminiImageResponseUnmarshalError("{}"), null);
  assert.equal(geminiImageResponseUnmarshalError('{"predictions":[]}'), null);

  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/images/generations", { headers: { "x-oneapi-request-id": "hop448-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop448-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.142" });
  await mergeModelRatio(new Store(e.DB), { "imagen-hop448": 1, "gemini-hop448-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.143" },
      body: JSON.stringify({
        name: "hop448-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey",
        models: "imagen-hop448,gemini-hop448-chat",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("chat-hop360-stay")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("empty-stay")) {
      return new Response(JSON.stringify({ predictions: [] }), { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
  ...
  try {
    const imageHit = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.144", "x-oneapi-request-id": "hop448-image-unmarshal" },
        body: JSON.stringify({ model: "imagen-hop448", prompt: "hello" }),
      }),
      e,
    );
    assert.equal(imageHit.res.status, 500, imageHit.text);
    ...
    assert.equal(
      imageErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop448-image-unmarshal"),
    );
    assert.equal(imageErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(imageErr.param, "");
    assert.equal(imageErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const imageArray = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.145", "x-oneapi-request-id": "hop448-image-array" },
        body: JSON.stringify({ model: "imagen-hop448", prompt: "as-array" }),
      }),
      e,
    );
    assert.equal(imageArray.res.status, 500, imageArray.text);
    ...
    assert.equal(
      imageArrayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiImageResponse", "hop448-image-array"),
    );
    assert.equal(imageArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const emptyStay = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.146", "x-oneapi-request-id": "hop448-empty-stay" },
        body: JSON.stringify({ model: "imagen-hop448", prompt: "empty-stay" }),
      }),
      e,
    );
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.147", "x-oneapi-request-id": "hop448-hop360-stay" },
        body: JSON.stringify({
          model: "gemini-hop448-chat",
          messages: [{ role: "user", content: "chat-hop360-stay" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      chatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop448-hop360-stay"),
    );
    assert.equal(chatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      chatStay.text.includes("dto.GeminiChatResponse") || chatErr.message.includes("dto.GeminiChatResponse"),
      false,
      chatStay.text,
    );
    ...
    const streamHit = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.148", "x-oneapi-request-id": "hop448-image-stream" },
        body: JSON.stringify({ model: "imagen-hop448", prompt: "hello-stream", stream: true }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 500, streamHit.text);
    ...
    assert.equal(
      streamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop448-image-stream"),
    );
    assert.equal(streamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover GeminiImageHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.149" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.151", "x-oneapi-request-id": "hop448-vendor-create" },
      body: JSON.stringify({ name: "hop448-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop448-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop448 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop448.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover GeminiEmbeddingHandler Unmarshal NewOpenAIError gin.H", async () => {
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop449"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop449", "embeddings"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop449", "chat"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "embedding-hop449", "embeddings"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini-embedding-hop449", "embeddings"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop449", "gemini"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop449", "responses"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_VERTEX, "text-embedding-hop449", "embeddings"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_OPENAI, "text-embedding-hop449", "embeddings"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop449", "embeddings"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini-hop449-chat", "chat"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop449", false), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop449", "embeddings", true), false);
  assert.equal(
    geminiEmbeddingResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    geminiEmbeddingResponseUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type dto.GeminiBatchEmbeddingResponse",
  );
  assert.equal(geminiEmbeddingResponseUnmarshalError("null"), null);
  assert.equal(geminiEmbeddingResponseUnmarshalError("{}"), null);
  assert.equal(geminiEmbeddingResponseUnmarshalError('{"embeddings":[]}'), null);

  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/embeddings", { headers: { "x-oneapi-request-id": "hop449-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop449-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.152" });
  await mergeModelRatio(new Store(e.DB), {
    "text-embedding-hop449": 1,
    "imagen-hop449": 1,
    "gemini-hop449-chat": 1,
  });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.153" },
      body: JSON.stringify({
        name: "hop449-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey",
        models: "text-embedding-hop449,imagen-hop449,gemini-hop449-chat",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("chat-hop449-stay")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("empty-embed-stay")) {
      return new Response(JSON.stringify({ embeddings: [] }), { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("as-array")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("image-hop448-stay")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  try {
    const embedHit = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.154", "x-oneapi-request-id": "hop449-embed-unmarshal" },
        body: JSON.stringify({ model: "text-embedding-hop449", input: "hello" }),
      }),
      e,
    );
    assert.equal(embedHit.res.status, 500, embedHit.text);
    ...
    assert.equal(
      embedErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop449-embed-unmarshal"),
    );
    assert.equal(embedErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(embedErr.param, "");
    assert.equal(embedErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      embedHit.text.includes("dto.GeminiChatResponse") || embedErr.message.includes("dto.GeminiChatResponse"),
      false,
      embedHit.text,
    );
    ...
    const embedArray = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.155", "x-oneapi-request-id": "hop449-embed-array" },
        body: JSON.stringify({ model: "text-embedding-hop449", input: "as-array" }),
      }),
      e,
    );
    assert.equal(embedArray.res.status, 500, embedArray.text);
    const embedArrayErr = embedArray.body.error as { message: string; type: string };
    assert.equal(
      embedArrayErr.message,
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiBatchEmbeddingResponse",
    ...
    assert.equal(embedArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const emptyStay = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.156", "x-oneapi-request-id": "hop449-empty-stay" },
        body: JSON.stringify({ model: "text-embedding-hop449", input: "empty-embed-stay" }),
      }),
      e,
    );
    assert.equal(emptyStay.res.status, 200, emptyStay.text);
    assert.equal(emptyStay.body.object, "list");
    assert.deepEqual(emptyStay.body.data, []);
    assert.equal(emptyStay.body.model, "text-embedding-hop449");

    const imageStay = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.157", "x-oneapi-request-id": "hop449-hop448-stay" },
        body: JSON.stringify({ model: "imagen-hop449", prompt: "image-hop448-stay" }),
      }),
      e,
    );
    assert.equal(imageStay.res.status, 500, imageStay.text);
    const imageErr = imageStay.body.error as { message: string; type: string };
    assert.equal(
      imageErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop449-hop448-stay"),
    );
    assert.equal(imageErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      imageStay.text.includes("dto.GeminiBatchEmbeddingResponse") || imageErr.message.includes("dto.GeminiBatchEmbeddingResponse"),
      false,
      imageStay.text,
    );
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.158", "x-oneapi-request-id": "hop449-hop360-stay" },
        body: JSON.stringify({
          model: "gemini-hop449-chat",
          messages: [{ role: "user", content: "chat-hop449-stay" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      chatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop449-hop360-stay"),
    );
    assert.equal(chatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      chatStay.text.includes("dto.GeminiBatchEmbeddingResponse") || chatErr.message.includes("dto.GeminiBatchEmbeddingResponse"),
      false,
      chatStay.text,
    );
    ...
    const streamHit = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.159", "x-oneapi-request-id": "hop449-embed-stream" },
        body: JSON.stringify({ model: "text-embedding-hop449", input: "hello-stream", stream: true }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 500, streamHit.text);
    ...
    assert.equal(
      streamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop449-embed-stream"),
    );
    assert.equal(streamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const chatAsEmbed = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.161", "x-oneapi-request-id": "hop449-chat-as-embed" },
        body: JSON.stringify({
          model: "text-embedding-hop449",
          messages: [{ role: "user", content: "chat-as-embed-hop449" }],
        }),
      }),
      e,
    );
    assert.equal(chatAsEmbed.res.status, 500, chatAsEmbed.text);
    const chatAsEmbedErr = chatAsEmbed.body.error as { message: string; type: string };
    assert.equal(
      chatAsEmbedErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop449-chat-as-embed"),
    );
    assert.equal(chatAsEmbedErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      chatAsEmbed.text.includes("dto.GeminiChatResponse") || chatAsEmbedErr.message.includes("dto.GeminiChatResponse"),
      false,
      chatAsEmbed.text,
    );
  } finally {
...
test("original leftover GeminiEmbeddingHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.162" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.163", "x-oneapi-request-id": "hop449-vendor-create" },
      body: JSON.stringify({ name: "hop449-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop449-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop449 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop449.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover NativeGeminiEmbeddingHandler Unmarshal NewOpenAIError gin.H", async () => {
  const embedPath = "/v1beta/models/text-embedding-hop450:embedContent";
  const batchPath = "/v1beta/models/text-embedding-hop450:batchEmbedContents";
  const chatPath = "/v1beta/models/gemini-hop450-chat:generateContent";
  ...
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini", chatPath), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "embeddings", "/v1/embeddings"), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini", "/v1/embeddings"), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_VERTEX, "gemini", embedPath), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_OPENAI, "gemini", embedPath), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop450", "gemini"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop450", "embeddings"), true);
  assert.equal(
    nativeGeminiEmbeddingResponseUnmarshalError("not-json", embedPath),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    nativeGeminiEmbeddingResponseUnmarshalError("[]", embedPath),
    "json: cannot unmarshal array into Go value of type dto.GeminiEmbeddingResponse",
  );
  assert.equal(
    nativeGeminiEmbeddingResponseUnmarshalError("[]", batchPath),
    "json: cannot unmarshal array into Go value of type dto.GeminiBatchEmbeddingResponse",
  );
  assert.equal(nativeGeminiEmbeddingResponseUnmarshalError("null", embedPath), null);
  assert.equal(nativeGeminiEmbeddingResponseUnmarshalError("{}", embedPath), null);
  assert.equal(nativeGeminiEmbeddingResponseUnmarshalError('{"embedding":{"values":[0.1]}}', embedPath), null);
  assert.equal(nativeGeminiEmbeddingResponseUnmarshalError('{"embeddings":[]}', batchPath), null);

  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local" + embedPath, { headers: { "x-oneapi-request-id": "hop450-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop450-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.164" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop450": 1, "gemini-hop450-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.165" },
      body: JSON.stringify({
        name: "hop450-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey",
        models: "text-embedding-hop450,gemini-hop450-chat",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("native-embed-stay")) {
      return new Response(JSON.stringify({ embedding: { values: [0.11, 0.22] } }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
  ...
  try {
    const embedHit = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.166", "x-oneapi-request-id": "hop450-embed-unmarshal" },
    ...
    assert.equal(
      embedErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop450-embed-unmarshal"),
    );
    assert.equal(embedErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(embedErr.param, "");
    assert.equal(embedErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    assert.equal(
      embedHit.text.includes("dto.GeminiBatchEmbeddingResponse") || embedErr.message.includes("dto.GeminiBatchEmbeddingResponse"),
      false,
      embedHit.text,
    );
    ...
    const embedArray = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.167", "x-oneapi-request-id": "hop450-embed-array" },
        body: JSON.stringify({ content: { parts: [{ text: "as-array" }] } }),
      }),
      e,
    );
    assert.equal(embedArray.res.status, 500, embedArray.text);
    ...
    assert.equal(
      embedArrayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiEmbeddingResponse", "hop450-embed-array"),
    );
    assert.equal(embedArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const batchHit = await send(
      new Request("http://local" + batchPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.168", "x-oneapi-request-id": "hop450-batch-unmarshal" },
        body: JSON.stringify({ requests: [{ content: { parts: [{ text: "hello-batch" }] } }] }),
      }),
      e,
    );
    assert.equal(batchHit.res.status, 500, batchHit.text);
    const batchErr = batchHit.body.error as { message: string; type: string };
    assert.equal(
      batchErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop450-batch-unmarshal"),
    );
    assert.equal(batchErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const batchArray = await send(
      new Request("http://local" + batchPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.169", "x-oneapi-request-id": "hop450-batch-array" },
        body: JSON.stringify({ requests: [{ content: { parts: [{ text: "as-array" }] } }] }),
      }),
      e,
    );
    assert.equal(batchArray.res.status, 500, batchArray.text);
    const batchArrayErr = batchArray.body.error as { message: string; type: string };
    assert.equal(
      batchArrayErr.message,
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiBatchEmbeddingResponse",
        "hop450-batch-array",
      ),
    );
    assert.equal(batchArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const nativeStay = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.171", "x-oneapi-request-id": "hop450-native-stay" },
        body: JSON.stringify({ content: { parts: [{ text: "native-embed-stay" }] } }),
      }),
      e,
    );
    assert.equal(nativeStay.res.status, 200, nativeStay.text);
    assert.deepEqual(nativeStay.body.embedding, { values: [0.11, 0.22] });
    assert.equal(nativeStay.body.object === "list", false, nativeStay.text);

    const hop449Stay = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.172", "x-oneapi-request-id": "hop450-hop449-stay" },
        body: JSON.stringify({ model: "text-embedding-hop450", input: "embed-hop449-stay" }),
      }),
      e,
    );
    assert.equal(hop449Stay.res.status, 500, hop449Stay.text);
    const hop449Err = hop449Stay.body.error as { message: string; type: string };
    assert.equal(
      hop449Err.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop450-hop449-stay"),
    );
    assert.equal(hop449Err.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      hop449Stay.text.includes("dto.GeminiEmbeddingResponse") || hop449Err.message.includes("dto.GeminiEmbeddingResponse"),
      false,
      hop449Stay.text,
    );
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.173", "x-oneapi-request-id": "hop450-hop360-stay" },
        body: JSON.stringify({
          model: "gemini-hop450-chat",
          messages: [{ role: "user", content: "chat-hop360-stay-450" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      chatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop450-hop360-stay"),
    );
    assert.equal(chatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      chatStay.text.includes("dto.GeminiEmbeddingResponse") || chatErr.message.includes("dto.GeminiEmbeddingResponse"),
      false,
      chatStay.text,
    );

    const nativeChatStay = await send(
      new Request("http://local" + chatPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.174", "x-oneapi-request-id": "hop450-native-chat-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-chat-hop360-stay" }] }] }),
      }),
      e,
    );
    assert.equal(nativeChatStay.res.status, 500, nativeChatStay.text);
    const nativeChatErr = nativeChatStay.body.error as { message: string; type: string };
    assert.equal(
      nativeChatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop450-native-chat-stay"),
    );
    assert.equal(nativeChatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      nativeChatStay.text.includes("dto.GeminiEmbeddingResponse") || nativeChatErr.message.includes("dto.GeminiEmbeddingResponse"),
      false,
      nativeChatStay.text,
    );
  } finally {
...
test("original leftover NativeGeminiEmbeddingHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.175" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.176", "x-oneapi-request-id": "hop450-vendor-create" },
      body: JSON.stringify({ name: "hop450-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop450-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop450 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop450.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom GeminiEmbeddingHandler Unmarshal NewOpenAIError gin.H", async () => {
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  const chatToClaude = "openai_chat_completions_to_anthropic_messages";
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "text-embedding-hop451"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "embedding-hop451"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "gemini-embedding-hop451"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "hop451-gemini"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "embeddings", chatToGemini, "text-embedding-hop451"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", chatToGemini, "text-embedding-hop451"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "text-embedding-hop451"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", "none", "text-embedding-hop451"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "chat", chatToGemini, "text-embedding-hop451"),
    false,
  );
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop451", "chat"), true);
  assert.equal(usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop451-gemini"), true);
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "text-embedding-hop451"),
    false,
  );

  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop451-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop451-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.177" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop451": 1, "hop451-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.178" },
      body: JSON.stringify({
        name: "hop451-adv-gemini",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop451",
        models: "text-embedding-hop451,hop451-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  try {
    const embedHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.179", "x-oneapi-request-id": "hop451-embed-unmarshal" },
        body: JSON.stringify({
          model: "text-embedding-hop451",
          messages: [{ role: "user", content: "hello" }],
        }),
      }),
      e,
    ...
    assert.equal(
      embedErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop451-embed-unmarshal"),
    );
    assert.equal(embedErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(embedErr.param, "");
    assert.equal(embedErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const embedArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.181", "x-oneapi-request-id": "hop451-embed-array" },
        body: JSON.stringify({
          model: "text-embedding-hop451",
          messages: [{ role: "user", content: "as-array" }],
        }),
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.182", "x-oneapi-request-id": "hop451-hop420-stay" },
        body: JSON.stringify({
          model: "hop451-gemini",
          messages: [{ role: "user", content: "chat-hop420-stay" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      chatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop451-hop420-stay"),
    );
    assert.equal(chatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      chatStay.text.includes("dto.GeminiBatchEmbeddingResponse") || chatErr.message.includes("dto.GeminiBatchEmbeddingResponse"),
    ...
    const streamHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.183", "x-oneapi-request-id": "hop451-embed-stream" },
        body: JSON.stringify({
          model: "text-embedding-hop451",
    ...
    assert.equal(
      streamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop451-embed-stream"),
    );
    assert.equal(streamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover advanced-custom GeminiEmbeddingHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.184" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.185", "x-oneapi-request-id": "hop451-vendor-create" },
      body: JSON.stringify({ name: "hop451-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop451-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop451 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop451.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom NativeGeminiEmbeddingHandler Unmarshal NewOpenAIError gin.H", async () => {
  const embedPath = "/v1beta/models/text-embedding-hop452:embedContent";
  const batchPath = "/v1beta/models/text-embedding-hop452:batchEmbedContents";
  const chatPath = "/v1beta/models/gemini-hop452-chat:generateContent";
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", embedPath, "none", "gemini"), true);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", batchPath, "none", "gemini"), true);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", embedPath, chatToGemini), true);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", embedPath), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", chatPath, "none", "gemini"), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", embedPath, "none", "gemini"), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini", embedPath), true);
  assert.equal(
    nativeGeminiEmbeddingResponseUnmarshalError("[]", embedPath),
    "json: cannot unmarshal array into Go value of type dto.GeminiEmbeddingResponse",
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local" + embedPath, { headers: { "x-oneapi-request-id": "hop452-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop452-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.186" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop452": 1, "gemini-hop452-chat": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.187" },
      body: JSON.stringify({
        name: "hop452-adv-gemini",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop452",
        models: "text-embedding-hop452,gemini-hop452-chat",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
            advanced_routes: [
              {
                ...
                converter: "none",
                models: ["text-embedding-hop452"],
              },
              {
                incoming_path: "/v1beta/models/{model}:batchEmbedContents",
                upstream_path: "https://generativelanguage.googleapis.com/v1beta/models/{model}:batchEmbedContents",
                converter: "none",
                models: ["text-embedding-hop452"],
              },
              {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("native-embed-stay-452")) {
      return new Response(JSON.stringify({ embedding: { values: [0.5, 0.6] } }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
  ...
  try {
    const embedHit = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.189", "x-oneapi-request-id": "hop452-embed-unmarshal" },
        body: JSON.stringify({ content: { parts: [{ text: "hello" }] } }),
      }),
      e,
    );
    ...
    assert.equal(
      embedErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop452-embed-unmarshal"),
    );
    assert.equal(embedErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(embedErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
    ...
    const embedArray = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.191", "x-oneapi-request-id": "hop452-embed-array" },
        body: JSON.stringify({ content: { parts: [{ text: "as-array" }] } }),
      }),
      e,
    );
    ...
    assert.equal(
      embedArrayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiEmbeddingResponse", "hop452-embed-array"),
    );

    const batchHit = await send(
      new Request("http://local" + batchPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.192", "x-oneapi-request-id": "hop452-batch-unmarshal" },
        body: JSON.stringify({ requests: [{ content: { parts: [{ text: "hello-batch" }] } }] }),
      }),
      e,
    );
    ...
    assert.equal(
      batchErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop452-batch-unmarshal"),
    );
    assert.equal(batchErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const nativeChatStay = await send(
      new Request("http://local" + chatPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.193", "x-oneapi-request-id": "hop452-native-chat-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-chat-hop360-stay-452" }] }] }),
      }),
      e,
    );
    assert.equal(nativeChatStay.res.status, 500, nativeChatStay.text);
    ...
    assert.equal(
      nativeChatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop452-native-chat-stay"),
    );
    assert.equal(nativeChatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      nativeChatStay.text.includes("dto.GeminiEmbeddingResponse") || nativeChatErr.message.includes("dto.GeminiEmbeddingResponse"),
    ...
    const nativeStay = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.194", "x-oneapi-request-id": "hop452-native-stay" },
        body: JSON.stringify({ content: { parts: [{ text: "native-embed-stay-452" }] } }),
      }),
      e,
    );
    assert.equal(nativeStay.res.status, 200, nativeStay.text);
    assert.deepEqual(nativeStay.body.embedding, { values: [0.5, 0.6] });
    assert.equal(nativeStay.body.object === "list", false, nativeStay.text);
  } finally {
    globalThis.fetch = origFetch;
  }
...
test("original leftover advanced-custom NativeGeminiEmbeddingHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.195" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.196", "x-oneapi-request-id": "hop452-vendor-create" },
      body: JSON.stringify({ name: "hop452-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop452-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop452 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop452.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom GeminiImageHandler Unmarshal NewOpenAIError gin.H", async () => {
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  const chatToClaude = "openai_chat_completions_to_anthropic_messages";
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "imagen-hop453"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "imagen-3.0-generate-001"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "hop453-gemini"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "images", chatToGemini, "imagen-hop453"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", chatToGemini, "imagen-hop453"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToClaude, "imagen-hop453"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", "none", "imagen-hop453"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "chat", chatToGemini, "imagen-hop453"),
    false,
  );
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop453", false), true);
  assert.equal(usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop453-gemini"), true);
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-hop453"),
    false,
  );
  assert.equal(
    geminiImageResponseUnmarshalError("[]"),
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop453-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop453-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.197" });
  await mergeModelRatio(new Store(e.DB), { "imagen-hop453": 1, "hop453-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.198" },
      body: JSON.stringify({
        name: "hop453-adv-imagen",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop453",
        models: "imagen-hop453,hop453-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  try {
    const imageHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.199", "x-oneapi-request-id": "hop453-image-unmarshal" },
        body: JSON.stringify({
          model: "imagen-hop453",
          messages: [{ role: "user", content: "hello" }],
        }),
    ...
    assert.equal(
      imageErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop453-image-unmarshal"),
    );
    assert.equal(imageErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(imageErr.param, "");
    assert.equal(imageErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      imageHit.text.includes("dto.GeminiChatResponse") || imageErr.message.includes("dto.GeminiChatResponse"),
      false,
      imageHit.text,
    );
    ...
    const imageArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.201", "x-oneapi-request-id": "hop453-image-array" },
        body: JSON.stringify({
          model: "imagen-hop453",
          messages: [{ role: "user", content: "as-array" }],
        }),
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.202", "x-oneapi-request-id": "hop453-hop420-stay" },
        body: JSON.stringify({
          model: "hop453-gemini",
          messages: [{ role: "user", content: "chat-hop420-stay" }],
        }),
    ...
    assert.equal(
      chatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop453-hop420-stay"),
    );
    assert.equal(chatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      chatStay.text.includes("dto.GeminiImageResponse") || chatErr.message.includes("dto.GeminiImageResponse"),
      false,
      chatStay.text,
    );
    ...
    const streamHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.203", "x-oneapi-request-id": "hop453-image-stream" },
        body: JSON.stringify({
          model: "imagen-hop453",
          messages: [{ role: "user", content: "hello-stream" }],
          stream: true,
    ...
    assert.equal(
      streamErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop453-image-stream"),
    );
    assert.equal(streamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
  } finally {
    globalThis.fetch = origFetch;
...
test("original leftover advanced-custom GeminiImageHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.204" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.205", "x-oneapi-request-id": "hop453-vendor-create" },
      body: JSON.stringify({ name: "hop453-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop453-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop453 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop453.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover native GeminiTextGenerationHandler Unmarshal NewOpenAIError gin.H for RelayModeGemini :predict imagen", async () => {
  const predictPath = "/v1beta/models/imagen-hop454:predict";
  const chatPath = "/v1beta/models/hop454-gemini:generateContent";
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454", false, "gemini"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454", true, "gemini"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454", false, "chat"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454", false, "images"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454"), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454", "gemini", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454", "chat", true), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop454", "images", true), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop454-gemini", "gemini", true), true);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini", predictPath), false);
  assert.equal(
    geminiChatResponseUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
  );
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local" + predictPath, { headers: { "x-oneapi-request-id": "hop454-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop454-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.206" });
  await mergeModelRatio(new Store(e.DB), { "imagen-hop454": 1, "hop454-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.207" },
      body: JSON.stringify({
        name: "hop454-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop454",
        models: "imagen-hop454,hop454-gemini",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("native-predict-stay")) {
      return new Response(JSON.stringify({ candidates: [{ content: { parts: [{ text: "ok" }] } }] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("stream-hop454")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    if (raw.includes("as-array")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
  ...
  try {
    ...
    assert.equal(predictHit.res.status, 500, predictHit.text);
    assert.equal("type" in predictHit.body && predictHit.body.type === "error", false, predictHit.text);
    const predictErr = predictHit.body.error as { message: string; type: string; param: string; code: string };
    assert.equal(
      predictErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop454-predict-unmarshal"),
    );
    assert.equal(predictErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(predictErr.param, "");
    assert.equal(predictErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      predictHit.text.includes("dto.GeminiImageResponse") || predictErr.message.includes("dto.GeminiImageResponse"),
      false,
      predictHit.text,
    );

    const predictArray = await send(
      new Request("http://local" + predictPath, {
        method: "POST",
    ...
    assert.equal(predictArray.res.status, 500, predictArray.text);
    const predictArrayErr = predictArray.body.error as { message: string; type: string };
    assert.equal(
      predictArrayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiChatResponse", "hop454-predict-array"),
    );
    assert.equal(predictArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      predictArray.text.includes("dto.GeminiImageResponse") || predictArrayErr.message.includes("dto.GeminiImageResponse"),
      false,
      predictArray.text,
    );
    ...
    const imageStay = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.211", "x-oneapi-request-id": "hop454-hop448-stay" },
        body: JSON.stringify({ model: "imagen-hop454", prompt: "as-array" }),
      }),
      e,
    );
    assert.equal(imageStay.res.status, 500, imageStay.text);
    ...
    assert.equal(
      ...
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiImageResponse", "hop454-hop448-stay"),
    );
    ...
    const nativeChatStay = await send(
      new Request("http://local" + chatPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.212", "x-oneapi-request-id": "hop454-hop360-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "as-array" }] }] }),
      }),
      e,
    );
    ...
    assert.equal(
      nativeChatErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiChatResponse", "hop454-hop360-stay"),
    );
    assert.equal(nativeChatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    assert.equal(nativeStay.res.status, 200, nativeStay.text);
    assert.ok(Array.isArray(nativeStay.body.candidates), nativeStay.text);
    assert.equal(nativeStay.body.object === "list", false, nativeStay.text);
...
test("original leftover native GeminiTextGenerationHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.215" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.216", "x-oneapi-request-id": "hop454-vendor-create" },
      body: JSON.stringify({ name: "hop454-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop454-vendor-create", { headers: auth }),
    e,
  );
...
test("original leftover GeminiResponsesHandler Unmarshal NewOpenAIError gin.H for /v1/responses imagen", async () => {
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", false, "responses"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", true, "responses"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", false, "images"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", false, "chat"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", false, "gemini"), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", "responses", true), true);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", "responses", false), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", "chat", true), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop457", "responses", true), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_VERTEX, "imagen-hop455", "chat", true), true);
  assert.equal(usesVolcTTSUnmarshal(CHANNEL_TYPE_VOLC, "audio_speech"), true);
  assert.equal(
    geminiChatResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(
    geminiChatResponseUnmarshalError("[]"),
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/responses", { headers: { "x-oneapi-request-id": "hop457-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop457-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.237" });
  await mergeModelRatio(new Store(e.DB), { "imagen-hop457": 1, "hop457-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.238" },
      body: JSON.stringify({
        name: "hop457-gemini-imagen",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop457",
        models: "imagen-hop457,hop457-gemini",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (raw.includes("stream-hop440-stay") || raw.includes("stream-hop457")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  try {
    const responsesHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.239", "x-oneapi-request-id": "hop457-responses-unmarshal" },
        body: JSON.stringify({
          model: "imagen-hop457",
          input: "hello",
        }),
    ...
    assert.equal(responsesHit.res.status, 500, responsesHit.text);
    assert.equal("type" in responsesHit.body && responsesHit.body.type === "error", false, responsesHit.text);
    const responsesErr = responsesHit.body.error as { message: string; type: string; param: string; code: string };
    assert.equal(
      responsesErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop457-responses-unmarshal"),
    );
    assert.equal(responsesErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(responsesErr.param, "");
    assert.equal(responsesErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      responsesHit.text.includes("dto.GeminiImageResponse") || responsesErr.message.includes("dto.GeminiImageResponse"),
      false,
      responsesHit.text,
    );
    ...
    const responsesArray = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.241", "x-oneapi-request-id": "hop457-responses-array" },
        body: JSON.stringify({
          model: "imagen-hop457",
          input: "as-array",
        }),
    ...
    assert.equal(responsesArray.res.status, 500, responsesArray.text);
    const responsesArrayErr = responsesArray.body.error as { message: string; type: string };
    assert.equal(
      responsesArrayErr.message,
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
        "hop457-responses-array",
      ),
    );
    assert.equal(responsesArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(responsesArrayErr.message.includes("dto.GeminiImageResponse"), false, responsesArray.text);

    const imageStay = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.242", "x-oneapi-request-id": "hop457-hop448-stay" },
        body: JSON.stringify({
          model: "imagen-hop457",
          prompt: "non-stream-hop448",
        }),
      }),
      e,
    );
    ...
    assert.equal(
      imageStayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiImageResponse", "hop457-hop448-stay"),
    );
    assert.equal(imageStayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const streamStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.243", "x-oneapi-request-id": "hop457-hop440-stay" },
        body: JSON.stringify({
          model: "hop457-gemini",
          stream: true,
          input: "stream-hop440-stay",
        }),
      }),
      e,
    );
    assert.equal(streamStay.res.status, 200, streamStay.text);
    assert.equal(streamStay.res.headers.get("content-type")?.includes("text/event-stream"), true, streamStay.text);
    assert.ok(
      streamStay.text.includes("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value"),
      streamStay.text,
    );
    assert.ok(streamStay.text.includes("event: error"), streamStay.text);
    assert.equal(
      streamStay.text.includes(messageWithRequestId("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value", "hop457-hop440-stay")),
      false,
      streamStay.text,
    );

    const streamImagen = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.244", "x-oneapi-request-id": "hop457-stream-imagen" },
        body: JSON.stringify({
          model: "imagen-hop457",
          stream: true,
          input: "stream-hop457",
    ...
    assert.equal(streamImagen.res.status, 200, streamImagen.text);
    assert.equal(streamImagen.res.headers.get("content-type")?.includes("text/event-stream"), true, streamImagen.text);
    assert.ok(
      streamImagen.text.includes("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value"),
      streamImagen.text,
    );
    assert.equal(streamImagen.text.includes("dto.GeminiImageResponse"), false, streamImagen.text);
    assert.equal(
      streamImagen.text.includes(messageWithRequestId("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value", "hop457-stream-imagen")),
      false,
      streamImagen.text,
    );
  } finally {
...
test("original leftover GeminiResponsesHandler /v1/responses imagen Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.246" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.247", "x-oneapi-request-id": "hop457-vendor-create" },
      body: JSON.stringify({ name: "hop457-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop457-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop457 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop457.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover GeminiResponsesHandler Unmarshal NewOpenAIError gin.H for /v1/responses embedding", async () => {
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "responses"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "embeddings"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "chat"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "gemini"), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "responses", true), true);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "responses", false), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "chat", true), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop458", "responses", true), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop458", false, "responses"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop458", false, "images"), true);
  assert.equal(usesVolcTTSUnmarshal(CHANNEL_TYPE_VOLC, "audio_speech"), true);
  assert.equal(
    geminiChatResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/responses", { headers: { "x-oneapi-request-id": "hop458-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop458-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.248" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop458": 1, "imagen-hop458": 1, "hop458-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.249" },
      body: JSON.stringify({
        name: "hop458-gemini-embed",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop458",
        models: "text-embedding-hop458,imagen-hop458,hop458-gemini",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("non-stream-hop449") || raw.includes("non-stream-hop457") || raw.includes("as-array")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("stream-hop458") || raw.includes("stream-hop440-stay")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  try {
    const responsesHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.251", "x-oneapi-request-id": "hop458-responses-unmarshal" },
        body: JSON.stringify({
          model: "text-embedding-hop458",
          input: "hello",
        }),
      }),
      e,
    ...
    assert.equal(
      responsesErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop458-responses-unmarshal"),
    );
    assert.equal(responsesErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(responsesErr.param, "");
    assert.equal(responsesErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      responsesHit.text.includes("dto.GeminiBatchEmbeddingResponse") ||
        responsesErr.message.includes("dto.GeminiBatchEmbeddingResponse"),
      false,
      responsesHit.text,
    );
    ...
    const responsesArray = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.252", "x-oneapi-request-id": "hop458-responses-array" },
        body: JSON.stringify({
          model: "text-embedding-hop458",
          input: "as-array",
        }),
    ...
    assert.equal(
      ...
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
        "hop458-responses-array",
      ),
    );
    assert.equal(responsesArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(responsesArrayErr.message.includes("dto.GeminiBatchEmbeddingResponse"), false, responsesArray.text);

    const embedStay = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.253", "x-oneapi-request-id": "hop458-hop449-stay" },
        body: JSON.stringify({
          model: "text-embedding-hop458",
          input: "non-stream-hop449",
        }),
      }),
      e,
    );
    assert.equal(embedStay.res.status, 500, embedStay.text);
    const embedStayErr = embedStay.body.error as { message: string; type: string };
    assert.equal(
      embedStayErr.message,
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiBatchEmbeddingResponse",
        "hop458-hop449-stay",
      ),
    );
    assert.equal(embedStayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const imagenStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.254", "x-oneapi-request-id": "hop458-hop457-stay" },
        body: JSON.stringify({
          model: "imagen-hop458",
          input: "non-stream-hop457",
        }),
      }),
      e,
    );
    assert.equal(imagenStay.res.status, 500, imagenStay.text);
    const imagenStayErr = imagenStay.body.error as { message: string; type: string };
    assert.equal(
      imagenStayErr.message,
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
        "hop458-hop457-stay",
      ),
    );
    assert.equal(imagenStayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);

    const streamEmbed = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.256", "x-oneapi-request-id": "hop458-stream-embed" },
        body: JSON.stringify({
          model: "text-embedding-hop458",
          stream: true,
          input: "stream-hop458",
    ...
    assert.equal(streamEmbed.res.status, 200, streamEmbed.text);
    assert.equal(streamEmbed.res.headers.get("content-type")?.includes("text/event-stream"), true, streamEmbed.text);
    assert.ok(
      streamEmbed.text.includes("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value"),
      streamEmbed.text,
    );
    assert.equal(streamEmbed.text.includes("dto.GeminiBatchEmbeddingResponse"), false, streamEmbed.text);
    assert.equal(
      streamEmbed.text.includes(
        messageWithRequestId(
          "unmarshal Gemini stream response: invalid character 'o' looking for beginning of value",
          "hop458-stream-embed",
        ),
...
test("original leftover GeminiResponsesHandler /v1/responses embedding Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.257" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.258", "x-oneapi-request-id": "hop458-vendor-create" },
      body: JSON.stringify({ name: "hop458-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop458-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop458 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop458.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom GeminiResponsesHandler Unmarshal NewOpenAIError gin.H for /v1/responses embedding", async () => {
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "text-embedding-hop459"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "imagen-hop459"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "text-embedding-hop459"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-hop459"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "embeddings", responsesToGemini, "openai", "text-embedding-hop459"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "text-embedding-hop459"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "text-embedding-hop459"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "imagen-hop459"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiResponsesStreamUnmarshal(
      ...
      responsesToGemini,
      "openai",
      "text-embedding-hop459",
      true,
    ),
    true,
  );
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop459", "responses"), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop459", "responses", true), true);
  assert.equal(usesVolcTTSUnmarshal(CHANNEL_TYPE_VOLC, "audio_speech"), true);
  assert.equal(
    advancedCustomGeminiResponseUnmarshalError("not-json"),
    "invalid character 'o' looking for beginning of value",
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/responses", { headers: { "x-oneapi-request-id": "hop459-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop459-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.259" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop459": 1, "imagen-hop459": 1, "hop459-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.261" },
      body: JSON.stringify({
        name: "hop459-adv-embed",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop459",
        models: "text-embedding-hop459,imagen-hop459,hop459-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("as-array") || raw.includes("non-stream-hop421") || raw.includes("non-stream-hop457")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("stream-hop459")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  try {
    const responsesHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.262", "x-oneapi-request-id": "hop459-adv-unmarshal" },
        body: JSON.stringify({
          model: "text-embedding-hop459",
          input: "hello",
        }),
      }),
      e,
    ...
    assert.equal(
      responsesErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop459-adv-unmarshal"),
    );
    assert.equal(responsesErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(responsesErr.param, "");
    assert.equal(responsesErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const responsesArray = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.263", "x-oneapi-request-id": "hop459-adv-array" },
        body: JSON.stringify({
          model: "text-embedding-hop459",
          input: "as-array",
        }),
    ...
    const chatStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.264", "x-oneapi-request-id": "hop459-hop421-stay" },
        body: JSON.stringify({
          model: "hop459-gemini",
          input: "non-stream-hop421",
        }),
      }),
      e,
    );
    ...
    const imagenStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.266", "x-oneapi-request-id": "hop459-hop457-stay" },
        body: JSON.stringify({
          model: "imagen-hop459",
          input: "non-stream-hop457",
        }),
    ...
    assert.equal(imagenStayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(imagenStayErr.message.includes("dto.GeminiImageResponse"), false, imagenStay.text);

    const streamHit = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.267", "x-oneapi-request-id": "hop459-stream-embed" },
        body: JSON.stringify({
          model: "text-embedding-hop459",
          stream: true,
          input: "stream-hop459",
    ...
    assert.equal(streamHit.res.status, 200, streamHit.text);
    assert.equal(streamHit.res.headers.get("content-type")?.includes("text/event-stream"), true, streamHit.text);
    assert.ok(
      streamHit.text.includes("unmarshal Gemini stream response: invalid character 'o' looking for beginning of value"),
      streamHit.text,
    );
    assert.equal(streamHit.text.includes("dto.GeminiBatchEmbeddingResponse"), false, streamHit.text);
    assert.equal(
      streamHit.text.includes(
        messageWithRequestId(
          "unmarshal Gemini stream response: invalid character 'o' looking for beginning of value",
          "hop459-stream-embed",
        ),
...
test("original leftover advanced-custom GeminiResponsesHandler /v1/responses embedding Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.268" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.269", "x-oneapi-request-id": "hop459-vendor-create" },
      body: JSON.stringify({ name: "hop459-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop459-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop459 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop459.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom native GeminiTextGenerationHandler Unmarshal NewOpenAIError gin.H for RelayModeGemini :predict imagen", async () => {
  const predictPath = "/v1beta/models/imagen-hop460:predict";
  const chatPath = "/v1beta/models/hop460-gemini:generateContent";
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "gemini", "imagen-hop460"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-hop460"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "imagen-hop460"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "images", "none", "gemini", "imagen-hop460"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "imagen-hop460"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "imagen-hop460"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "gemini", "imagen-hop460", true),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-hop460", true),
    false,
  );
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop460", false, "gemini"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop460", false, "images"), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop460", "gemini", true), true);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", predictPath, "none", "gemini"), false);
  assert.equal(usesVolcTTSUnmarshal(CHANNEL_TYPE_VOLC, "audio_speech"), true);
  assert.equal(
    geminiChatResponseUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local" + predictPath, { headers: { "x-oneapi-request-id": "hop460-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop460-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.271" });
  await mergeModelRatio(new Store(e.DB), { "imagen-hop460": 1, "hop460-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.272" },
      body: JSON.stringify({
        name: "hop460-adv-predict",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop460",
        models: "imagen-hop460,hop460-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
            advanced_routes: [
              {
                incoming_path: "/v1beta/models/{model}:predict",
                upstream_path: "https://generativelanguage.googleapis.com/v1beta/models/{model}:predict",
                converter: "none",
                models: ["imagen-hop460"],
              },
              {
                incoming_path: "/v1beta/models/{model}:generateContent",
                upstream_path: "https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent",
                converter: "none",
                models: ["hop460-gemini"],
              },
            ],
          },
        }),
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("native-predict-stay-460")) {
      return new Response(JSON.stringify({ candidates: [{ content: { parts: [{ text: "ok" }] } }] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("stream-hop460")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    if (raw.includes("as-array") || raw.includes("native-chat-hop360-stay-460")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  try {
    const predictHit = await send(
      new Request("http://local" + predictPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.273", "x-oneapi-request-id": "hop460-predict-unmarshal" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "hello" }] }] }),
      }),
      e,
    );
    assert.equal(predictHit.res.status, 500, predictHit.text);
    const predictErr = predictHit.body.error as { message: string; type: string; code: string };
    assert.equal(
      predictErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop460-predict-unmarshal"),
    );
    assert.equal(predictErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(predictErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(predictHit.text.includes("dto.GeminiImageResponse"), false, predictHit.text);

    const predictArray = await send(
      new Request("http://local" + predictPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.274", "x-oneapi-request-id": "hop460-predict-array" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "as-array" }] }] }),
      }),
      e,
    );
    assert.equal(predictArray.res.status, 500, predictArray.text);
    const arrayErr = predictArray.body.error as { message: string; type: string };
    assert.equal(
      arrayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiChatResponse", "hop460-predict-array"),
    );
    assert.equal(arrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(arrayErr.message.includes("dto.GeminiImageResponse"), false, predictArray.text);

    const nativeChatStay = await send(
      new Request("http://local" + chatPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.276", "x-oneapi-request-id": "hop460-hop452-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-chat-hop360-stay-460" }] }] }),
      }),
      e,
    );
    assert.equal(nativeChatStay.res.status, 500, nativeChatStay.text);
    ...
    assert.equal(nativeChatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(nativeChatStay.text.includes("dto.GeminiImageResponse"), false, nativeChatStay.text);

    const nativeStay = await send(
      new Request("http://local" + predictPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.277", "x-oneapi-request-id": "hop460-native-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-predict-stay-460" }] }] }),
      }),
      e,
    );
    assert.equal(nativeStay.res.status, 200, nativeStay.text);
    ...
    const streamHit = await send(
      new Request("http://local" + predictPath, {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.278",
          "x-oneapi-request-id": "hop460-predict-stream",
        },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "stream-hop460" }] }], stream: true }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 500, streamHit.text);
    ...
    assert.equal(
      ...
      messageWithRequestId(
        "unmarshal Gemini stream response: invalid character 'o' looking for beginning of value",
        "hop460-predict-stream",
      ),
    );
    assert.equal(streamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(streamHit.text.includes("dto.GeminiImageResponse"), false, streamHit.text);
  } finally {
    globalThis.fetch = origFetch;
  }
});

test("original leftover advanced-custom native GeminiTextGenerationHandler :predict imagen Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.279" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.281", "x-oneapi-request-id": "hop460-vendor-create" },
      body: JSON.stringify({ name: "hop460-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop460-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop460 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop460.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover native GeminiTextGenerationHandler Unmarshal NewOpenAIError gin.H for RelayModeGemini :generateContent embedding prefixes", async () => {
  const genPath = "/v1beta/models/text-embedding-hop461:generateContent";
  const embedPath = "/v1beta/models/text-embedding-hop461:embedContent";
  const chatPath = "/v1beta/models/hop461-gemini:generateContent";
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop461", "gemini"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop461", "embeddings"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop461", "chat"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop461", "responses"), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini", genPath), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "gemini", embedPath), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop461", "gemini", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop461", "chat", true), false);
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local" + genPath, { headers: { "x-oneapi-request-id": "hop461-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop461-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.282" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop461": 1, "hop461-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.283" },
      body: JSON.stringify({
        name: "hop461-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop461",
        models: "text-embedding-hop461,hop461-gemini",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("native-gen-stay-461")) {
      return new Response(JSON.stringify({ candidates: [{ content: { parts: [{ text: "ok" }] } }] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("stream-hop461")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    if (
      raw.includes("as-array") ||
      raw.includes("native-chat-hop360-stay-461") ||
      raw.includes("as-array-hop450-stay-461") ||
      raw.includes("as-array-hop449-stay-461")
    ) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  ...
  try {
    const genHit = await send(
      new Request("http://local" + genPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.284", "x-oneapi-request-id": "hop461-gen-unmarshal" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "hello" }] }] }),
      }),
      e,
    );
    assert.equal(genHit.res.status, 500, genHit.text);
    assert.equal("type" in genHit.body && genHit.body.type === "error", false, genHit.text);
    const genErr = genHit.body.error as { message: string; type: string; param: string; code: string };
    assert.equal(
      genErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop461-gen-unmarshal"),
    );
    assert.equal(genErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(genErr.param, "");
    assert.equal(genErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      genHit.text.includes("dto.GeminiEmbeddingResponse") || genErr.message.includes("dto.GeminiEmbeddingResponse"),
      false,
      genHit.text,
    );
    assert.equal(
      genHit.text.includes("dto.GeminiBatchEmbeddingResponse") || genErr.message.includes("dto.GeminiBatchEmbeddingResponse"),
      false,
      genHit.text,
    );

    const genArray = await send(
      new Request("http://local" + genPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.286", "x-oneapi-request-id": "hop461-gen-array" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "as-array" }] }] }),
      }),
      e,
    );
    assert.equal(genArray.res.status, 500, genArray.text);
    const genArrayErr = genArray.body.error as { message: string; type: string };
    assert.equal(
      genArrayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiChatResponse", "hop461-gen-array"),
    );
    assert.equal(genArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(genArrayErr.message.includes("dto.GeminiEmbeddingResponse"), false, genArray.text);
    assert.equal(genArrayErr.message.includes("dto.GeminiBatchEmbeddingResponse"), false, genArray.text);

    const hop450Stay = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.287", "x-oneapi-request-id": "hop461-hop450-stay" },
        body: JSON.stringify({ content: { parts: [{ text: "as-array-hop450-stay-461" }] } }),
      }),
      e,
    );
    assert.equal(hop450Stay.res.status, 500, hop450Stay.text);
    const hop450Err = hop450Stay.body.error as { message: string; type: string };
    assert.equal(
      hop450Err.message,
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiEmbeddingResponse",
    ...
    assert.equal(hop450Err.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(hop450Stay.text.includes("dto.GeminiChatResponse"), false, hop450Stay.text);

    const hop449Stay = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.288", "x-oneapi-request-id": "hop461-hop449-stay" },
        body: JSON.stringify({ model: "text-embedding-hop461", input: "as-array-hop449-stay-461" }),
      }),
      e,
    );
    assert.equal(hop449Stay.res.status, 500, hop449Stay.text);
    ...
    assert.equal(hop449Err.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(hop449Stay.text.includes("dto.GeminiChatResponse"), false, hop449Stay.text);

    const nativeChatStay = await send(
      new Request("http://local" + chatPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.289", "x-oneapi-request-id": "hop461-hop360-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-chat-hop360-stay-461" }] }] }),
      }),
      e,
    );
    assert.equal(nativeChatStay.res.status, 500, nativeChatStay.text);
    ...
    assert.equal(nativeChatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(nativeChatStay.text.includes("dto.GeminiEmbeddingResponse"), false, nativeChatStay.text);

    const nativeStay = await send(
      new Request("http://local" + genPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.291", "x-oneapi-request-id": "hop461-native-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-gen-stay-461" }] }] }),
      }),
      e,
    );
    assert.equal(nativeStay.res.status, 200, nativeStay.text);
    ...
    const streamHit = await send(
      new Request("http://local" + genPath, {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.292",
          "x-oneapi-request-id": "hop461-gen-stream",
        },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "stream-hop461" }] }], stream: true }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 500, streamHit.text);
    ...
    assert.equal(streamErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(streamHit.text.includes("dto.GeminiEmbeddingResponse"), false, streamHit.text);
  } finally {
    globalThis.fetch = origFetch;
  }
});

test("original leftover native GeminiTextGenerationHandler :generateContent embedding Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.293" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.294", "x-oneapi-request-id": "hop461-vendor-create" },
      body: JSON.stringify({ name: "hop461-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop461-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop461 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop461.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom native GeminiTextGenerationHandler Unmarshal NewOpenAIError gin.H for RelayModeGemini :generateContent embedding prefixes", async () => {
  const genPath = "/v1beta/models/text-embedding-hop462:generateContent";
  const embedPath = "/v1beta/models/text-embedding-hop462:embedContent";
  const chatPath = "/v1beta/models/hop462-gemini:generateContent";
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "gemini", "text-embedding-hop462"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "text-embedding-hop462"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "text-embedding-hop462"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "embeddings", "none", "gemini", "text-embedding-hop462"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "text-embedding-hop462"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "text-embedding-hop462"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "gemini", "text-embedding-hop462", true),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiStreamUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "text-embedding-hop462", true),
    false,
  );
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop462", "gemini", true), true);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", genPath, "none", "gemini"), false);
  assert.equal(usesNativeGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", embedPath, "none", "gemini"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop462", "gemini"), false);
  assert.equal(
    geminiChatResponseUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
  );
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local" + genPath, { headers: { "x-oneapi-request-id": "hop462-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop462-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.296" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop462": 1, "hop462-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.297" },
      body: JSON.stringify({
        name: "hop462-adv-gen",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop462",
        models: "text-embedding-hop462,hop462-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("native-gen-stay-462")) {
      return new Response(JSON.stringify({ candidates: [{ content: { parts: [{ text: "ok" }] } }] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("stream-hop462")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    if (
      raw.includes("as-array") ||
      raw.includes("native-chat-hop360-stay-462") ||
      raw.includes("as-array-hop452-stay-462")
    ) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  ...
  try {
    const genHit = await send(
      new Request("http://local" + genPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.298", "x-oneapi-request-id": "hop462-gen-unmarshal" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "hello" }] }] }),
      }),
      e,
    );
    assert.equal(genHit.res.status, 500, genHit.text);
    const genErr = genHit.body.error as { message: string; type: string };
    assert.equal(
      genErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop462-gen-unmarshal"),
    );
    assert.equal(genErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(genHit.text.includes("dto.GeminiEmbeddingResponse"), false, genHit.text);
    assert.equal(genHit.text.includes("dto.GeminiBatchEmbeddingResponse"), false, genHit.text);

    const genArray = await send(
      new Request("http://local" + genPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.299", "x-oneapi-request-id": "hop462-gen-array" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "as-array" }] }] }),
      }),
      e,
    );
    ...
    assert.equal(
      genArrayErr.message,
      messageWithRequestId("json: cannot unmarshal array into Go value of type dto.GeminiChatResponse", "hop462-gen-array"),
    );
    assert.equal(genArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(genArrayErr.message.includes("dto.GeminiEmbeddingResponse"), false, genArray.text);
    assert.equal(genArrayErr.message.includes("dto.GeminiBatchEmbeddingResponse"), false, genArray.text);

    const hop452Stay = await send(
      new Request("http://local" + embedPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.301", "x-oneapi-request-id": "hop462-hop452-stay" },
        body: JSON.stringify({ content: { parts: [{ text: "as-array-hop452-stay-462" }] } }),
      }),
      e,
    );
    assert.equal(hop452Stay.res.status, 500, hop452Stay.text);
    const hop452Err = hop452Stay.body.error as { message: string; type: string };
    assert.equal(
      hop452Err.message,
      messageWithRequestId(
        "json: cannot unmarshal array into Go value of type dto.GeminiEmbeddingResponse",
    ...
    assert.equal(hop452Err.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(hop452Stay.text.includes("dto.GeminiChatResponse"), false, hop452Stay.text);

    const nativeChatStay = await send(
      new Request("http://local" + chatPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.302", "x-oneapi-request-id": "hop462-hop360-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-chat-hop360-stay-462" }] }] }),
      }),
      e,
    );
    assert.equal(nativeChatStay.res.status, 500, nativeChatStay.text);
    ...
    const nativeStay = await send(
      new Request("http://local" + genPath, {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.303", "x-oneapi-request-id": "hop462-native-stay" },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "native-gen-stay-462" }] }] }),
      }),
      e,
    );
    assert.equal(nativeStay.res.status, 200, nativeStay.text);
    ...
    const streamHit = await send(
      new Request("http://local" + genPath, {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.304",
          "x-oneapi-request-id": "hop462-gen-stream",
        },
        body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: "stream-hop462" }] }], stream: true }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 500, streamHit.text);
...
test("original leftover advanced-custom native GeminiTextGenerationHandler :generateContent embedding Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.306" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.307", "x-oneapi-request-id": "hop462-vendor-create" },
      body: JSON.stringify({ name: "hop462-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop462-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop462 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop462.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover Vertex RequestModeGemini embedding GeminiChatHandler Unmarshal NewOpenAIError gin.H", async () => {
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop463", "chat"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_VERTEX, "text-embedding-hop463", "chat"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop463", "gemini"), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_VERTEX, "text-embedding-hop463", "chat", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_VERTEX, "text-embedding-hop463", "completions", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_VERTEX, "text-embedding-hop463", "chat", false), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_VERTEX, "text-embedding-hop463", "gemini", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop463", "chat", true), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop463", "embeddings", true), false);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop463", "gemini", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_VERTEX, "imagen-hop455", "chat", true), true);
  assert.equal(vertexRequestMode("text-embedding-hop463"), "gemini");
  assert.equal(
    geminiChatResponseUnmarshalError("[]"),
    "json: cannot unmarshal array into Go value of type dto.GeminiChatResponse",
  );
  ...
  const helper = writeGeminiChatUnmarshalError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop463-helper" } }),
    "invalid character 'o' looking for beginning of value",
  );
  assert.equal(helper.status, 500);
  assert.deepEqual(await helper.json(), {
    error: {
      message: messageWithRequestId("invalid character 'o' looking for beginning of value", "hop463-helper"),
      type: ERROR_CODE_BAD_RESPONSE_BODY,
      param: "",
      code: ERROR_CODE_BAD_RESPONSE_BODY,
    },
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.308" });
  await mergeModelRatio(new Store(e.DB), { "text-embedding-hop463": 1, "hop463-gemini": 1 });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const created = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.309" },
      body: JSON.stringify({
        name: "hop463-vertex-embed",
        type: CHANNEL_TYPE_VERTEX,
        key: "vkey-hop463",
        models: "text-embedding-hop463,hop463-gemini",
        group: "default",
        other: JSON.stringify({ default: "us-central1" }),
        settings: { vertex_key_type: "api_key" },
      }),
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("stream-hop463")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    if (raw.includes("as-array") || raw.includes("chat-hop360-stay-463")) {
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    }
    return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  try {
    const chatHit = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.311", "x-oneapi-request-id": "hop463-vertex-unmarshal" },
        body: JSON.stringify({
          model: "text-embedding-hop463",
          messages: [{ role: "user", content: "hello" }],
        }),
      }),
      e,
    ...
    assert.equal(
      chatErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop463-vertex-unmarshal"),
    );
    assert.equal(chatErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(chatErr.param, "");
    assert.equal(chatErr.code, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(
      chatHit.text.includes("dto.GeminiBatchEmbeddingResponse") || chatErr.message.includes("dto.GeminiBatchEmbeddingResponse"),
      false,
      chatHit.text,
    );
    assert.equal(
      chatHit.text.includes("dto.GeminiEmbeddingResponse") || chatErr.message.includes("dto.GeminiEmbeddingResponse"),
      false,
      chatHit.text,
    );
    ...
    const chatArray = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.312", "x-oneapi-request-id": "hop463-vertex-array" },
        body: JSON.stringify({
          model: "text-embedding-hop463",
          messages: [{ role: "user", content: "as-array" }],
        }),
    ...
    assert.equal(chatArrayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(chatArrayErr.message.includes("dto.GeminiBatchEmbeddingResponse"), false, chatArray.text);

    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.313", "x-oneapi-request-id": "hop463-hop360-stay" },
        body: JSON.stringify({
          model: "hop463-gemini",
          messages: [{ role: "user", content: "chat-hop360-stay-463" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(chatStayErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(chatStay.text.includes("dto.GeminiEmbeddingResponse"), false, chatStay.text);

    const embedConvert = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.314", "x-oneapi-request-id": "hop463-hop350-stay" },
        body: JSON.stringify({ model: "text-embedding-hop463", input: "embed-hop350-stay-463" }),
      }),
      e,
    );
    ...
    const streamHit = await send(
      new Request("http://local/v1/chat/completions", {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.316",
          "x-oneapi-request-id": "hop463-vertex-stream",
        },
        body: JSON.stringify({
          model: "text-embedding-hop463",
          stream: true,
          messages: [{ role: "user", content: "stream-hop463" }],
        }),
      }),
      e,
    );
...
test("original leftover Vertex RequestModeGemini embedding GeminiChatHandler Unmarshal gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.317" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.318", "x-oneapi-request-id": "hop463-vendor-create" },
      body: JSON.stringify({ name: "hop463-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop463-vendor-create", { headers: auth }),
    e,
  );
...
test("original leftover advanced-custom GeminiResponsesHandler empty-candidates NewOpenAIError gin.H", async () => {
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "hop465-gemini"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "imagen-hop465"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "text-embedding-hop465"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop465-gemini"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "gemini", "none", "gemini", "hop465-gemini"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "text-embedding-hop465"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "imagen-hop465"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiResponsesStreamUnmarshal(
  ...
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop465", false, "responses"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop465", "responses"), false);
  assert.equal(usesGeminiResponsesStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop465-gemini-stay", "responses", true), true);
  assert.equal(
    geminiChatEmptyCandidatesError({ candidates: [] })?.code,
    ERROR_CODE_EMPTY_RESPONSE,
  );
  ...
  const helper = writeGeminiChatEmptyCandidatesError(
    new Request("http://local/v1/responses", { headers: { "x-oneapi-request-id": "hop465-helper" } }),
    500,
    "empty response from Gemini API",
    ERROR_CODE_EMPTY_RESPONSE,
  );
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.332" });
  await mergeModelRatio(new Store(e.DB), {
    "hop465-gemini": 1,
    "imagen-hop465": 1,
    "text-embedding-hop465": 1,
    "hop465-gemini-stay": 1,
  });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const createdAdv = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.333" },
      body: JSON.stringify({
        name: "hop465-adv-responses",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop465",
        models: "hop465-gemini,imagen-hop465,text-embedding-hop465",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        status_code_mapping: JSON.stringify({ "500": "503" }),
        settings: JSON.stringify({
  ...
  const createdGemini = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.334" },
      body: JSON.stringify({
        name: "hop465-gemini-stay",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop465",
        models: "hop465-gemini-stay",
        group: "default",
        status_code_mapping: JSON.stringify({ "500": "503" }),
      }),
    }),
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("block-me-465")) {
      return new Response(JSON.stringify({ candidates: [], promptFeedback: { blockReason: "SAFETY" } }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("not-json-hop459-stay-465")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("stream-hop441-stay-465")) {
      return new Response("data: not-json\n\n", { status: 200, headers: { "content-type": "text/event-stream" } });
    }
    return new Response(JSON.stringify({ candidates: [] }), {
      status: 200,
  ...
  try {
    const empty = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.336", "x-oneapi-request-id": "hop465-responses-empty" },
        body: JSON.stringify({ model: "hop465-gemini", input: "hello" }),
      }),
      e,
    );
    assert.equal(empty.res.status, 503, empty.text);
    ...
    assert.equal(emptyErr.param, "");
    assert.equal(emptyErr.code, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(empty.text.includes("(request id: hop465-responses-empty)"), false, empty.text);

    const blocked = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.337", "x-oneapi-request-id": "hop465-responses-blocked" },
        body: JSON.stringify({ model: "hop465-gemini", input: "block-me-465" }),
      }),
      e,
    );
    assert.equal(blocked.res.status, 400, blocked.text);
    ...
    assert.equal(blockedErr.type, ERROR_CODE_PROMPT_BLOCKED);
    assert.equal(blockedErr.code, ERROR_CODE_PROMPT_BLOCKED);
    assert.equal(blocked.text.includes("(request id: hop465-responses-blocked)"), false, blocked.text);

    const imagenEmpty = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.338", "x-oneapi-request-id": "hop465-imagen-empty" },
        body: JSON.stringify({ model: "imagen-hop465", input: "hello-imagen" }),
      }),
      e,
    );
    assert.equal(imagenEmpty.res.status, 503, imagenEmpty.text);
    ...
    const embedEmpty = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.339", "x-oneapi-request-id": "hop465-embed-empty" },
        body: JSON.stringify({ model: "text-embedding-hop465", input: "hello-embed" }),
      }),
      e,
    );
    assert.equal(embedEmpty.res.status, 503, embedEmpty.text);
    ...
    const geminiStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.341", "x-oneapi-request-id": "hop465-hop464-stay" },
        body: JSON.stringify({ model: "hop465-gemini-stay", input: "hello-hop464-stay" }),
      }),
      e,
    );
    assert.equal(geminiStay.res.status, 503, geminiStay.text);
    const stayErr = geminiStay.body.error as { message: string; type: string };
    assert.equal(stayErr.message, "empty response from Gemini API");
    assert.equal(stayErr.type, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(geminiStay.text.includes("(request id: hop465-hop464-stay)"), false, geminiStay.text);

    const unmarshalStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.342", "x-oneapi-request-id": "hop465-hop459-stay" },
        body: JSON.stringify({ model: "hop465-gemini", input: "not-json-hop459-stay-465" }),
      }),
      e,
    );
    ...
    assert.equal(
      ...
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop465-hop459-stay"),
    );
    ...
    const streamStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.343", "x-oneapi-request-id": "hop465-hop441-stay" },
        body: JSON.stringify({ model: "hop465-gemini", input: "stream-hop441-stay-465", stream: true }),
      }),
      e,
    );
    assert.equal(streamStay.res.status, 200, streamStay.text);
    assert.equal(streamStay.text.includes("empty_response"), false, streamStay.text);
    assert.equal(streamStay.text.includes("(request id: hop465-hop441-stay)"), false, streamStay.text);
  } finally {
    globalThis.fetch = origFetch;
  }
});

test("original leftover advanced-custom GeminiResponsesHandler empty-candidates gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.344" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.346", "x-oneapi-request-id": "hop465-vendor-create" },
      body: JSON.stringify({ name: "hop465-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop465-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop465 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop465.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom GeminiChatHandler empty-candidates NewOpenAIError gin.H", async () => {
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop466-gemini"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-hop466"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "text-embedding-hop466"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "hop466-gemini"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "imagen-hop466"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "text-embedding-hop466"),
    true,
  );
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "hop466-gemini-stay", "chat", true), true);
  assert.equal(
    geminiChatEmptyCandidatesError({ candidates: [] })?.code,
    ERROR_CODE_EMPTY_RESPONSE,
  );
  ...
  const helper = writeGeminiChatEmptyCandidatesError(
    new Request("http://local/v1/chat/completions", { headers: { "x-oneapi-request-id": "hop466-helper" } }),
    500,
    "empty response from Gemini API",
    ERROR_CODE_EMPTY_RESPONSE,
  );
  ...
  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.347" });
  await mergeModelRatio(new Store(e.DB), {
    "hop466-gemini": 1,
    "imagen-hop466": 1,
    "text-embedding-hop466": 1,
    "hop466-gemini-stay": 1,
  });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const createdAdv = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.348" },
      body: JSON.stringify({
        name: "hop466-adv-chat",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop466",
        models: "hop466-gemini,imagen-hop466,text-embedding-hop466",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        status_code_mapping: JSON.stringify({ "500": "503" }),
        settings: JSON.stringify({
  ...
  const createdGemini = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.349" },
      body: JSON.stringify({
        name: "hop466-gemini-stay",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop466",
        models: "hop466-gemini-stay",
        group: "default",
        status_code_mapping: JSON.stringify({ "500": "503" }),
      }),
    }),
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("block-me-466")) {
      return new Response(JSON.stringify({ candidates: [], promptFeedback: { blockReason: "SAFETY" } }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("not-json-hop420-stay-466")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    return new Response(JSON.stringify({ candidates: [] }), {
      status: 200,
  ...
  try {
    const empty = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.351", "x-oneapi-request-id": "hop466-chat-empty" },
        body: JSON.stringify({
          model: "hop466-gemini",
          messages: [{ role: "user", content: "hello" }],
        }),
    ...
    assert.equal(emptyErr.param, "");
    assert.equal(emptyErr.code, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(empty.text.includes("(request id: hop466-chat-empty)"), false, empty.text);

    const blocked = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.352", "x-oneapi-request-id": "hop466-chat-blocked" },
        body: JSON.stringify({
          model: "hop466-gemini",
          messages: [{ role: "user", content: "block-me-466" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(blockedErr.type, ERROR_CODE_PROMPT_BLOCKED);
    assert.equal(blockedErr.code, ERROR_CODE_PROMPT_BLOCKED);
    assert.equal(blocked.text.includes("(request id: hop466-chat-blocked)"), false, blocked.text);

    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.353", "x-oneapi-request-id": "hop466-hop359-stay" },
        body: JSON.stringify({
          model: "hop466-gemini-stay",
          messages: [{ role: "user", content: "chat-hop359-stay-466" }],
        }),
      }),
      e,
    );
    assert.equal(chatStay.res.status, 503, chatStay.text);
    const stayErr = chatStay.body.error as { message: string; type: string };
    assert.equal(stayErr.message, "empty response from Gemini API");
    assert.equal(stayErr.type, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(chatStay.text.includes("(request id: hop466-hop359-stay)"), false, chatStay.text);

    const responsesStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.354", "x-oneapi-request-id": "hop466-hop465-stay" },
        body: JSON.stringify({ model: "hop466-gemini", input: "hello-hop465-stay" }),
      }),
      e,
    );
    assert.equal(responsesStay.res.status, 503, responsesStay.text);
    const responsesErr = responsesStay.body.error as { message: string; type: string };
    assert.equal(responsesErr.message, "empty response from Gemini API");
    assert.equal(responsesErr.type, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(responsesStay.text.includes("(request id: hop466-hop465-stay)"), false, responsesStay.text);

    const unmarshalStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.356", "x-oneapi-request-id": "hop466-hop420-stay" },
        body: JSON.stringify({
          model: "hop466-gemini",
          messages: [{ role: "user", content: "not-json-hop420-stay-466" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      unmarshalErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop466-hop420-stay"),
    );
    assert.equal(unmarshalErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(unmarshalStay.text.includes("empty_response"), false, unmarshalStay.text);
  } finally {
...
test("original leftover advanced-custom GeminiChatHandler empty-candidates gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.357" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.358", "x-oneapi-request-id": "hop466-vendor-create" },
      body: JSON.stringify({ name: "hop466-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop466-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop466 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop466.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover advanced-custom GeminiImageHandler empty predictions NewOpenAIError gin.H", async () => {
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  const responsesToGemini = "openai_responses_to_gemini_generate_content";
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "imagen-hop467"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-hop467"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "responses", responsesToGemini, "openai", "imagen-hop467"),
    true,
  );
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop467-stay", false, "images"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop467-stay", false, "responses"), false);

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.359" });
  await mergeModelRatio(new Store(e.DB), {
    "imagen-hop467": 1,
    "hop467-gemini": 1,
    "imagen-hop467-stay": 1,
  });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const createdAdv = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.361" },
      body: JSON.stringify({
        name: "hop467-adv-imagen",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop467",
        models: "imagen-hop467,hop467-gemini",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  const createdGemini = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.362" },
      body: JSON.stringify({
        name: "hop467-gemini-stay",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop467",
        models: "imagen-hop467-stay",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("not-json-hop453-stay-467")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("chat-hop466-stay-467")) {
      return new Response(JSON.stringify({ candidates: [] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("hello-hop465-stay")) {
      return new Response(JSON.stringify({ candidates: [] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
  ...
  try {
    const empty = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.363", "x-oneapi-request-id": "hop467-imagen-empty" },
        body: JSON.stringify({
          model: "imagen-hop467",
          messages: [{ role: "user", content: "hello" }],
        }),
    ...
    const imageStay = await send(
      new Request("http://local/v1/images/generations", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.364", "x-oneapi-request-id": "hop467-hop448-stay" },
        body: JSON.stringify({ model: "imagen-hop467-stay", prompt: "image-hop448-stay-467" }),
      }),
      e,
    );
    assert.equal(imageStay.res.status, 500, imageStay.text);
    ...
    const unmarshalStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.366", "x-oneapi-request-id": "hop467-hop453-stay" },
        body: JSON.stringify({
          model: "imagen-hop467",
          messages: [{ role: "user", content: "not-json-hop453-stay-467" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      unmarshalErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop467-hop453-stay"),
    );
    assert.equal(unmarshalErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.367", "x-oneapi-request-id": "hop467-hop466-stay" },
        body: JSON.stringify({
          model: "hop467-gemini",
          messages: [{ role: "user", content: "chat-hop466-stay-467" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(chatErr.message, "empty response from Gemini API");
    assert.equal(chatErr.type, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(chatStay.text.includes("(request id: hop467-hop466-stay)"), false, chatStay.text);
    assert.equal(chatStay.text.includes("no images generated"), false, chatStay.text);

    const responsesStay = await send(
      new Request("http://local/v1/responses", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.368", "x-oneapi-request-id": "hop467-hop465-stay" },
        body: JSON.stringify({ model: "imagen-hop467", input: "hello-hop465-stay" }),
      }),
      e,
    );
...
test("original leftover advanced-custom GeminiImageHandler empty predictions gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.369" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.371", "x-oneapi-request-id": "hop467-vendor-create" },
      body: JSON.stringify({ name: "hop467-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop467-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop467 = ((listed.body.data as { items: { action: string }[] }).items || []);
  assert.ok(vendorItemsHop467.some((item) => item.action === "vendor.create"), listed.text);
});

test("original leftover GeminiEmbeddingHandler convert OpenAI embedding JSON for chat embedding prefixes", async () => {
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop468", "chat"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop468", "embeddings"), true);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_GEMINI, "text-embedding-hop468", "responses"), false);
  assert.equal(usesGeminiEmbeddingUnmarshal(CHANNEL_TYPE_VERTEX, "text-embedding-hop468", "chat"), false);
  assert.equal(
    usesAdvancedCustomGeminiEmbeddingUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "text-embedding-hop468-adv"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "text-embedding-hop468-adv"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop468-gemini-adv"),
    true,
  );

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.372" });
  await mergeModelRatio(new Store(e.DB), {
    "text-embedding-hop468": 1,
    "hop468-gemini": 1,
    "text-embedding-hop468-adv": 1,
    "hop468-gemini-adv": 1,
  });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const createdGemini = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.373" },
      body: JSON.stringify({
        name: "hop468-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop468",
        models: "text-embedding-hop468,hop468-gemini",
        group: "default",
      }),
    }),
    e,
  ...
  const createdAdv = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.374" },
      body: JSON.stringify({
        name: "hop468-adv-embed",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop468",
        models: "text-embedding-hop468-adv,hop468-gemini-adv",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("empty-embed-468")) {
      return new Response(JSON.stringify({ embeddings: [] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("not-json-hop451-stay-468")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("chat-hop466-stay-468")) {
      return new Response(JSON.stringify({ candidates: [] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
  ...
  try {
    const chatEmbed = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.376", "x-oneapi-request-id": "hop468-chat-embed" },
        body: JSON.stringify({
          model: "text-embedding-hop468",
          messages: [{ role: "user", content: "hello-embed" }],
        }),
      }),
      e,
    );
    assert.equal(chatEmbed.res.status, 200, chatEmbed.text);
    assert.equal(chatEmbed.body.object, "list");
    assert.equal(chatEmbed.body.model, "text-embedding-hop468");
    assert.deepEqual(chatEmbed.body.data, [{ object: "embedding", embedding: [0.1, 0.2], index: 0 }]);
    assert.equal("choices" in chatEmbed.body, false, chatEmbed.text);

    const emptyEmbed = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.377", "x-oneapi-request-id": "hop468-chat-empty" },
        body: JSON.stringify({
          model: "text-embedding-hop468",
          messages: [{ role: "user", content: "empty-embed-468" }],
        }),
      }),
      e,
    );
    assert.equal(emptyEmbed.res.status, 200, emptyEmbed.text);
    assert.equal(emptyEmbed.body.object, "list");
    assert.deepEqual(emptyEmbed.body.data, []);

    const embeddingsStay = await send(
      new Request("http://local/v1/embeddings", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.378", "x-oneapi-request-id": "hop468-hop449-stay" },
        body: JSON.stringify({ model: "text-embedding-hop468", input: "hello-embed-stay" }),
      }),
      e,
    );
    assert.equal(embeddingsStay.res.status, 200, embeddingsStay.text);
    assert.equal(embeddingsStay.body.object, "list");
    assert.deepEqual(embeddingsStay.body.data, [{ object: "embedding", embedding: [0.1, 0.2], index: 0 }]);

    const advEmbed = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.379", "x-oneapi-request-id": "hop468-adv-embed" },
        body: JSON.stringify({
          model: "text-embedding-hop468-adv",
          messages: [{ role: "user", content: "hello-adv-embed" }],
        }),
      }),
      e,
    );
    assert.equal(advEmbed.res.status, 200, advEmbed.text);
    assert.equal(advEmbed.body.object, "list");
    assert.equal(advEmbed.body.model, "text-embedding-hop468-adv");
    assert.deepEqual(advEmbed.body.data, [{ object: "embedding", embedding: [0.1, 0.2], index: 0 }]);
    assert.equal("choices" in advEmbed.body, false, advEmbed.text);

    const unmarshalStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.381", "x-oneapi-request-id": "hop468-hop451-stay" },
        body: JSON.stringify({
          model: "text-embedding-hop468-adv",
          messages: [{ role: "user", content: "not-json-hop451-stay-468" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(
      unmarshalErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop468-hop451-stay"),
    );
    assert.equal(unmarshalErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.382", "x-oneapi-request-id": "hop468-hop466-stay" },
        body: JSON.stringify({
          model: "hop468-gemini",
          messages: [{ role: "user", content: "chat-hop466-stay-468" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(chatErr.message, "empty response from Gemini API");
    assert.equal(chatErr.type, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(chatStay.text.includes("(request id: hop468-hop466-stay)"), false, chatStay.text);
...
test("original leftover GeminiEmbeddingHandler convert gin.H does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.383" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.384", "x-oneapi-request-id": "hop468-vendor-create" },
      body: JSON.stringify({ name: "hop468-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop468-vendor-create", { headers: auth }),
    e,
  );
...
test("original leftover GeminiImageHandler write OpenAI image JSON even when the client streams", async () => {
  const chatToGemini = "openai_chat_completions_to_gemini_generate_content";
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop472", true, "images"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop472", false, "images"), true);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop472", true, "responses"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop472", true, "gemini"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_VERTEX, "imagen-hop472", true, "images"), false);
  assert.equal(usesGeminiImageUnmarshal(CHANNEL_TYPE_VERTEX, "imagen-hop472", false, "images"), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_VERTEX, "imagen-hop472", "images", true), true);
  assert.equal(usesGeminiChatStreamUnmarshal(CHANNEL_TYPE_GEMINI, "imagen-hop472", "images", true), false);
  assert.equal(
    usesAdvancedCustomGeminiImageUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "imagen-hop472-adv"),
    true,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "imagen-hop472-adv"),
    false,
  );
  assert.equal(
    usesAdvancedCustomGeminiUnmarshal(CHANNEL_TYPE_ADVANCED_CUSTOM, "chat", chatToGemini, "openai", "hop472-gemini-adv"),
    true,
  );

  resetSchemaFlag();
  const e = env();
  const { auth, sk } = await boot(e, { "cf-connecting-ip": "198.51.100.434" });
  await mergeModelRatio(new Store(e.DB), {
    "imagen-hop472": 1,
    "hop472-gemini": 1,
    "imagen-hop472-adv": 1,
    "hop472-gemini-adv": 1,
    "imagen-hop472-stay": 1,
  });
  const skAuth = { authorization: "Bearer " + sk, "content-type": "application/json" };
  const createdGemini = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.436" },
      body: JSON.stringify({
        name: "hop472-gemini",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop472",
        models: "imagen-hop472,hop472-gemini",
        group: "default",
      }),
    }),
    e,
  ...
  const createdAdv = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.437" },
      body: JSON.stringify({
        name: "hop472-adv-imagen",
        type: CHANNEL_TYPE_ADVANCED_CUSTOM,
        key: "sk-hop472",
        models: "imagen-hop472-adv,hop472-gemini-adv",
        group: "default",
        base_url: "https://generativelanguage.googleapis.com",
        settings: JSON.stringify({
          advanced_custom: {
  ...
  const createdStay = await send(
    new Request("http://local/api/channel/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.438" },
      body: JSON.stringify({
        name: "hop472-gemini-stay",
        type: CHANNEL_TYPE_GEMINI,
        key: "gkey-hop472-stay",
        models: "imagen-hop472-stay",
        group: "default",
      }),
    }),
    e,
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.includes("generativelanguage.googleapis.com")) return origFetch(input, init);
    const raw = typeof init?.body === "string" ? init.body : "";
    if (raw.includes("not-json-hop448-stay-472")) {
      return new Response("not-json", { status: 200, headers: { "content-type": "application/json" } });
    }
    if (raw.includes("empty-pred-hop470-stay-472")) {
      return new Response(JSON.stringify({ predictions: [] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (raw.includes("chat-hop466-stay-472")) {
      return new Response(JSON.stringify({ candidates: [] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    return new Response(JSON.stringify({ predictions: [{ bytesBase64Encoded: "YQ==" }] }), {
      status: 200,
      headers: { "content-type": "application/json" },
    });
  }) as typeof fetch;
  try {
    const streamHit = await send(
      new Request("http://local/v1/images/generations", {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.439",
          "x-oneapi-request-id": "hop472-stream-json",
        },
        body: JSON.stringify({ model: "imagen-hop472", prompt: "hello-stream", stream: true }),
      }),
      e,
    );
    assert.equal(streamHit.res.status, 200, streamHit.text);
    ...
    assert.equal(Array.isArray(streamHit.body.data), true, streamHit.text);
    const streamData = streamHit.body.data as { b64_json: string }[];
    assert.equal(streamData[0]?.b64_json, "YQ==");
    assert.equal(streamHit.text.startsWith("data:"), false, streamHit.text);
    assert.equal(streamHit.text.includes("no images generated"), false, streamHit.text);
    ...
    const chatStream = await send(
      new Request("http://local/v1/chat/completions", {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.441",
          "x-oneapi-request-id": "hop472-chat-stream",
        },
        body: JSON.stringify({
          model: "imagen-hop472",
    ...
    assert.equal((chatStream.res.headers.get("content-type") || "").includes("application/json"), true, String(chatStream.res.headers.get("content-type")));
    assert.equal((chatStream.res.headers.get("content-type") || "").includes("text/event-stream"), false, String(chatStream.res.headers.get("content-type")));
    const chatData = chatStream.body.data as { b64_json: string }[];
    assert.equal(chatData[0]?.b64_json, "YQ==");
    assert.equal("choices" in chatStream.body, false, chatStream.text);
    assert.equal(chatStream.text.startsWith("data:"), false, chatStream.text);
    ...
    const advStream = await send(
      new Request("http://local/v1/chat/completions", {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.442",
          "x-oneapi-request-id": "hop472-adv-stream",
        },
        body: JSON.stringify({
          model: "imagen-hop472-adv",
          messages: [{ role: "user", content: "hello-adv-stream" }],
          stream: true,
        }),
      }),
      e,
    ...
    assert.equal((advStream.res.headers.get("content-type") || "").includes("application/json"), true, String(advStream.res.headers.get("content-type")));
    assert.equal((advStream.res.headers.get("content-type") || "").includes("text/event-stream"), false, String(advStream.res.headers.get("content-type")));
    const advData = advStream.body.data as { b64_json: string }[];
    assert.equal(advData[0]?.b64_json, "YQ==");
    assert.equal("choices" in advStream.body, false, advStream.text);
    assert.equal(advStream.text.startsWith("data:"), false, advStream.text);
    ...
    const emptyStay = await send(
      new Request("http://local/v1/images/generations", {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.443",
          "x-oneapi-request-id": "hop472-hop470-stay",
        },
        body: JSON.stringify({ model: "imagen-hop472", prompt: "empty-pred-hop470-stay-472", stream: true }),
      }),
      e,
    );
    assert.equal(emptyStay.res.status, 500, emptyStay.text);
    const emptyErr = emptyStay.body.error as { message: string; type: string };
    assert.equal(emptyErr.message, "no images generated");
    assert.equal(emptyErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(emptyStay.text.includes("(request id: hop472-hop470-stay)"), false, emptyStay.text);
    ...
    const unmarshalStay = await send(
      new Request("http://local/v1/images/generations", {
        ...
        headers: {
          ...skAuth,
          accept: "text/event-stream",
          "cf-connecting-ip": "198.51.100.444",
          "x-oneapi-request-id": "hop472-hop448-stay",
        },
        body: JSON.stringify({ model: "imagen-hop472-stay", prompt: "not-json-hop448-stay-472", stream: true }),
      }),
      e,
    );
    assert.equal(unmarshalStay.res.status, 500, unmarshalStay.text);
    ...
    assert.equal(
      unmarshalErr.message,
      messageWithRequestId("invalid character 'o' looking for beginning of value", "hop472-hop448-stay"),
    );
    assert.equal(unmarshalErr.type, ERROR_CODE_BAD_RESPONSE_BODY);
    assert.equal(unmarshalStay.text.includes("no images generated"), false, unmarshalStay.text);
    ...
    const chatStay = await send(
      new Request("http://local/v1/chat/completions", {
        method: "POST",
        headers: { ...skAuth, "cf-connecting-ip": "198.51.100.446", "x-oneapi-request-id": "hop472-hop466-stay" },
        body: JSON.stringify({
          model: "hop472-gemini",
          messages: [{ role: "user", content: "chat-hop466-stay-472" }],
        }),
      }),
      e,
    );
    ...
    assert.equal(chatErr.message, "empty response from Gemini API");
    assert.equal(chatErr.type, ERROR_CODE_EMPTY_RESPONSE);
    assert.equal(chatStay.text.includes("(request id: hop472-hop466-stay)"), false, chatStay.text);
  } finally {
    globalThis.fetch = origFetch;
  }
});

test("original leftover GeminiImageHandler stream JSON write does not change AUTH StatusText or hop 323 vendor.create", async () => {
  resetSchemaFlag();
  const e = env();
  const { auth } = await boot(e, { "cf-connecting-ip": "198.51.100.447" });
  ...
  const created = await send(
    new Request("http://local/api/vendors/", {
      method: "POST",
      headers: { ...auth, "cf-connecting-ip": "198.51.100.448", "x-oneapi-request-id": "hop472-vendor-create" },
      body: JSON.stringify({ name: "hop472-vendor-create", description: "d", icon: "" }),
    }),
    e,
  );
  assert.equal(created.body.success, true, created.text);
  const listed = await send(
    new Request("http://local/api/audit?page_size=100&request_id=hop472-vendor-create", { headers: auth }),
    e,
  );
  const vendorItemsHop472 = ((listed.body.data as { items: { action: string }[] }).items || []);
