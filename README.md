...
- `GET /v1/realtime` WebSocket 升级代理（原 gorilla Dial：`wss://`、Azure `api-key`、preview-only `openai-beta`、Advanced Custom 路由 `https`→`wss`；OpenaiRealtimeHandler `RealtimeUsage` + PreWss/PostWss 额度）
...
- Azure / OpenAI 兼容 / Anthropic / Gemini / Ollama / Cloudflare（含 Workers AI STT `ConvertAudioRequest` 原始文件字节 + cfSTTHandler `{text}`） / 阿里兼容模式 / 智谱 / 讯飞 SparkDesk（WebSocket HTTP 101） / 火山（含 TTS WebSocket 二进制协议） / 自定义 URL
- OpenAI 兼容渠道 multipart 音频按原 adaptor 重写；Cloudflare STT 按原 adaptor 只转发 `file` 字节且不带 multipart Content-Type
...
Vertex ADC 使用原始 HTTP RS256 JWT + `oauth2/v4/token`（无 Google SDK）。AWS AKSK 使用原始 HTTP Bedrock `InvokeModel` / SigV4（service `bedrock`，无 AWS SDK）；`aws_key_type=api_key` 仍走 Converse URL。配置了兼容 Base URL 的仍按 HTTP 中继。
