 ...
 * `openai.OpenaiHandler` (native adaptor) unmarshal also uses this envelope. Extra-OK: Jina embeddings `openai.OpenaiHandler` (native adaptor) unmarshal also uses this envelope. Extra-OK: Vertex RequestModeOpenSource `openai.OpenaiHandler` (native adaptor) unmarshal also uses this envelope. Extra-OK: Perplexity `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: Siliconflow non-rerank `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: Deepseek OpenAI-format `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: Moonshot OpenAI-format `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: Baidu V2 `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: Ali non-image/rerank `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: Ali image `aliImageHandler` unmarshal also uses this envelope.
