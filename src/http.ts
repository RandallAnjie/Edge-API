 ...
 * `ollamaChatHandler` unmarshal also uses this envelope. Extra-OK: Ollama OpenAI-format stream `ollamaStreamHandler` unmarshal also uses this envelope. Extra-OK: ClaudeResponsesStreamHandler Unmarshal typically `FailResponsesStream` SSE (hop 439 leftover `NewOpenAIError` gin.H only when unhandled). Extra-OK: GeminiResponsesStreamHandler Unmarshal typically `FailResponsesStream` SSE (hop 440 leftover `NewOpenAIError` gin.H only when unhandled). Extra-OK: advanced-custom responses-to-Gemini `GeminiResponsesStreamHandler` Unmarshal typically `FailResponsesStream` SSE (hop 441 leftover `NewOpenAIError` gin.H only when unhandled). Extra-OK: advanced-custom `OaiChatToResponsesStreamHandler` Unmarshal typically `FailResponsesStream` SSE (hop 442 leftover `NewOpenAIError` gin.H only when unhandled). Extra-OK: advanced-custom `OaiChatToResponsesHandler` unmarshal uses this envelope (hop 443). Extra-OK: advanced-custom `OaiResponsesToChatStreamHandler` unmarshal also uses this envelope (hop 444). Extra-OK: advanced-custom `OaiResponsesToChatHandler` unmarshal also uses this envelope (hop 445). Extra-OK: native via-responses `OaiResponsesToChatBufferedStreamHandler` unmarshal also uses this envelope (hop 446). Extra-OK: `openaiImageJSONAsStreamHandler` unmarshal also uses this envelope (hop 447). Extra-OK: xAI `OpenaiImageHandler` unmarshal also uses this envelope (hop 475). Extra-OK: Zhipu v4 OpenAI-format `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: newapi OpenAI-format `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: sub2api OpenAI-format `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: advanced-custom OpenAI-shaped `openai.Adaptor.DoResponse` (native adaptor) unmarshal also uses this envelope. Extra-OK: Codex `OaiResponsesHandler` / `OaiResponsesCompactionHandler` unmarshal also uses this envelope. Extra-OK: Claude `HandleClaudeResponseData` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: AWS API-key `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Vertex RequestModeClaude `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Moonshot Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: MiniMax Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Deepseek Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Zhipu v4 Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: newapi Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: sub2api Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Ollama Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Ali anthropic-messages Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Volc special-base Claude-format `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: AWS AKSK non-Nova `awsHandler` `HandleClaudeResponseData` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: advanced-custom chat-to-Claude / ConverterNone+RelayFormatClaude `claude.Adaptor.DoResponse` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: advanced-custom chat-to-Gemini / ConverterNone+RelayFormatGemini `gemini.Adaptor.DoResponse` unmarshal uses `NewOpenAIError` + `writeGeminiChatUnmarshalError` instead. Extra-OK: advanced-custom responses-to-Gemini `GeminiResponsesHandler` unmarshal uses `NewOpenAIError` + `writeGeminiChatUnmarshalError` instead. Extra-OK: Claude stream `HandleStreamResponseData` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: AWS AKSK `awsStreamHandler` `HandleStreamResponseData` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: AWS API-key stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Vertex RequestModeClaude stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Moonshot Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: MiniMax Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Deepseek Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Zhipu v4 Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: newapi Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: sub2api Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Ollama Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Ali anthropic-messages Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Volc special-base Claude-format stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: advanced-custom chat-to-Claude / ConverterNone+RelayFormatClaude stream `claude.Adaptor` `ClaudeStreamHandler` unmarshal uses `NewError` + `writeRelayNewAPIError` instead. Extra-OK: Zhipu v4
 ...
 * `GeminiImageHandler` unmarshal also uses this envelope (hop 448). Extra-OK: `GeminiEmbeddingHandler` unmarshal also uses this envelope (hop 449). Extra-OK: `NativeGeminiEmbeddingHandler` unmarshal also uses this envelope (hop 450). Extra-OK: advanced-custom chat-to-Gemini `GeminiEmbeddingHandler` unmarshal also uses this envelope (hop 451). Extra-OK: advanced-custom native `NativeGeminiEmbeddingHandler` unmarshal also uses this envelope (hop 452). Extra-OK: advanced-custom chat-to-Gemini `GeminiImageHandler` unmarshal also uses this envelope (hop 453). Extra-OK: native `RelayModeGemini` `:predict` imagen `GeminiTextGenerationHandler` unmarshal also uses this envelope (hop 454). Extra-OK: Vertex RequestModeGemini stream imagen `GeminiChatStreamHandler` unmarshal also uses this envelope (hop 455). Extra-OK: `/v1/responses` imagen `GeminiResponsesHandler` unmarshal also uses this envelope (hop 457). Extra-OK: `/v1/responses` embedding-model `GeminiResponsesHandler` unmarshal also uses this envelope (hop 458). Extra-OK: advanced-custom `/v1/responses` embedding / imagen `GeminiResponsesHandler` unmarshal also uses this envelope (hop 459). Extra-OK: advanced-custom native `RelayModeGemini` `:predict` imagen `GeminiTextGenerationHandler` unmarshal also uses this envelope (hop 460). Extra-OK: native `RelayModeGemini` `:generateContent` embedding-model `GeminiTextGenerationHandler` unmarshal also uses this envelope (hop 461). Extra-OK: advanced-custom native `RelayModeGemini` `:generateContent` embedding-model `GeminiTextGenerationHandler` unmarshal also uses this envelope (hop 462). Extra-OK: Vertex RequestModeGemini OpenAI-format embedding-model `GeminiChatHandler` unmarshal also uses this envelope (hop 463).
 */
