  ...
  for (let retry = 0; retry <= retryTimes; retry++) {
    ...
    if (isSSE && res.body) {
      ...
      if (usesOpenaiImageJSONAsStreamUnmarshal(channel.type, mode, opts.stream, ct)) {
        ...
        const wrapped = openaiImageJSONAsStreamSse(parsedStream);
        extra.actualImageCount = wrapped.imageCount;
        const usage = usageFromOpenAI({ usage: wrapped.usage });
        ...
        return new Response(wrapped.sse, {
          status: 200,
          headers: {
