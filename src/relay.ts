...
import { geminiChatEmptyCandidatesError, geminiChatResponseUnmarshalError, geminiChatStreamSseUnmarshalError, geminiEmbeddingResponseUnmarshalError, geminiImageResponseUnmarshalError, geminiUpstreamToOpenAIChat, usesGeminiChatStreamUnmarshal, usesGeminiEmbeddingUnmarshal, usesGeminiImageUnmarshal } from "./gemini-response.js";
...
export async function relay(opts: RelayRequest): Promise<Response> {
  ...
  for (let retry = 0; retry <= retryTimes; retry++) {
    ...
    if (isSSE && res.body) {
      ...
      if (usesAwsAkskClaudeStreamUnmarshal(channel.type, mapped, mode, channel.settings, opts.stream)) {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesAwsClaudeStreamUnmarshal(channel.type, mode, channel.settings, opts.stream)) {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesVertexClaudeStreamUnmarshal(channel.type, mode, mapped, opts.stream)) {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesMoonshotClaudeStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesMiniMaxClaudeStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesDeepseekClaudeStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesZhipuV4ClaudeStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesNewApiClaudeStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesSub2apiClaudeStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesOllamaClaudeStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesAliClaudeStreamUnmarshal(channel.type, mode, mapped, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesVolcClaudeStreamUnmarshal(channel.type, mode, channel.base_url, opts.stream) && clientFormat === "anthropic") {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesAdvancedCustomClaudeStreamUnmarshal(channel.type, mode, advancedConverter || "none", clientFormat, opts.stream)) {
        const streamText = await res.text();
        const unmarshalErr = claudeStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesAdvancedCustomGeminiStreamUnmarshal(channel.type, mode, advancedConverter || "none", clientFormat, mapped, opts.stream)) {
        const streamText = await res.text();
        const unmarshalErr = geminiChatStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesOllamaStreamUnmarshal(channel.type, mode, opts.stream) && clientFormat === "openai") {
        const streamText = await res.text();
        const unmarshalErr = ollamaStreamUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
      ...
      if (usesOaiResponsesToChatStreamUnmarshal(channel.type, mode, advancedConverter || "none", opts.stream)) {
        const streamText = await res.text();
        const unmarshalErr = oaiResponsesToChatStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
          return writeOpenaiHandlerUnmarshalError(opts.req, unmarshalErr);
        }
      ...
      if (usesOaiResponsesToChatBufferedStreamUnmarshal(viaResponses, opts.stream)) {
        const streamText = await res.text();
        const unmarshalErr = oaiResponsesToChatBufferedStreamSseUnmarshalError(streamText);
        if (unmarshalErr) {
      ...
      if (usesOpenaiImageJSONAsStreamUnmarshal(channel.type, mode, opts.stream, ct)) {
        const streamText = await res.text();
        const unmarshalErr = openaiImageJSONAsStreamResponseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
          return writeOpenaiHandlerUnmarshalError(opts.req, unmarshalErr);
        }
      ...
      if (usesGeminiEmbeddingUnmarshal(channel.type, mapped, mode)) {
        const streamText = await res.text();
        const unmarshalErr = geminiEmbeddingResponseUnmarshalError(streamText);
        if (unmarshalErr) {
          await settle(store, auth, channel, model, promptEst, 0, useTime, true, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
          return writeGeminiChatUnmarshalError(opts.req, unmarshalErr);
        }
    ...
    if (usesGeminiEmbeddingUnmarshal(channel.type, mapped, mode)) {
      const unmarshalErr = geminiEmbeddingResponseUnmarshalError(text);
      if (unmarshalErr) {
        await settle(store, auth, channel, model, promptEst, 0, useTime, false, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
        return writeGeminiChatUnmarshalError(opts.req, unmarshalErr);
      }
    ...
    if (usesOaiChatToResponsesUnmarshal(channel.type, mode, advancedConverter || "none", opts.stream)) {
      const unmarshalErr = oaiChatToResponsesResponseUnmarshalError(text);
      if (unmarshalErr) {
        await settle(store, auth, channel, model, promptEst, 0, useTime, false, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
        return writeOpenaiHandlerUnmarshalError(opts.req, unmarshalErr);
      }
    }
    if (usesOaiResponsesToChatUnmarshal(channel.type, mode, advancedConverter || "none", opts.stream)) {
      const unmarshalErr = oaiResponsesToChatResponseUnmarshalError(text);
      if (unmarshalErr) {
        await settle(store, auth, channel, model, promptEst, 0, useTime, false, ip, rid, false, unmarshalErr.slice(0, 2000), extra);
        return writeOpenaiHandlerUnmarshalError(opts.req, unmarshalErr);
      }
    ...
    try {
      ...
      if (usesGeminiEmbeddingUnmarshal(channel.type, mapped, mode) && (parsed == null || typeof parsed !== "object" || Array.isArray(parsed))) {
        parsed = {};
      }
      if (
        usesOpenAIAdaptor(channel.type) &&
